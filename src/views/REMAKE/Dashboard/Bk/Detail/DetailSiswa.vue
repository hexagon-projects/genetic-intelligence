...
const getPlacementData = async () => {
  ...
  try {
    ...
    if (placementData.value.length > 0) {
      ...
      console.log(selectedInstitutionId.value);
    }
  } catch (error) {
...
const filteredSidebarItems = computed(() => {
  return sidebarItems.value.filter((item) => {
    if (["SMP", "SMK", "SMA"].includes(institutionType.value)) {
      return item.text !== "Hasil Tes CPM";
    } else if (["TK", "SD"].includes(institutionType.value)) {
...
const getDataSiswa = async () => {
  try {
    ...
    // console.log(`response`, response.data.data[0].institutions.type)
    siswaDatas.value = response.data.data[0];
    institutionType.value = response.data.data[0].institutions.type;
  } catch (error) {
    Swal.fire({
      icon: "error",
      title: "Error",
...
onBeforeMount(() => {
  ...
  if (!encodedStudentId) {
    router.push({ name: "bk.views.pengguna" });
    return;
  }
