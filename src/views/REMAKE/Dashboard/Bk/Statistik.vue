          ...
          >
            <div class="flex justify-start items-center gap-4">
              ...
              <div class="flex-grow flex flex-col justify-start items-start">
                <div class="h-6">
                  <div class="text-[#344053] text-xs font-sora leading-normal">
                    Total Test Jatidiri Cerdas<span
                      v-if="
                        institutionType == 'TK' ||
                        institutionType == 'SD' ||
                        institutionType == 'SDLB'
                      "
                    >
                      CPM</span
                    >
        ...
        <div class="bg-white p-4 flex flex-col rounded-xl mt-10">
          ...
          >
            <div class="text-[#0c141c] font-['Roboto'] leading-loose space-y-4">
              ...
              <div
                v-if="data_desc.length == 0"
                class="bg-[#F4F3FF] p-5 rounded-lg"
              >
                <span
                  class="mx-auto text-black text-base font-roboto font-normal"
                >
...
const data_pie_2 = ref({
  ...
  chartOptions: {
    ...
    colors: ["#1ab7ea"],
    xaxis: {
      categories: ["> 119", "111 - 119", "91 - 110", "80 - 90", "< 80"],
      labels: {
        style: {
          colors: "#666",
          fontSize: "14px",
...
const fetchDataStatusAPI = async () => {
  ...
  try {
    const response = await initAPI(
      "get",
      `bk/data-stats/${selectedInstitutionId.value}`,
      null,
      token
    );
    ...
    data_pie_2.value.series = [
      { name: "Jumlah", data: res.data_iq_result?.counts ?? [] },
    ];

    arrAssessment.value.map(
      (item, i) =>
        (item.persentase =
          Math.floor(res.data_assessment_result.percentage[i]) + "%")
    );
    arrIq.value.map(
      (item, i) =>
        (item.persentase = Math.floor(res.data_iq_result.percentage[i]) + "%")
    );

    if (res.data_gim_result.desc) {
      data_desc.value.push([
        Math.floor(Math.max(...res.data_gim_result.percentage)),
        res.data_gim_result.desc,
      ]);
    }

    if (res.data_assessment_result.desc) {
      parsedDesc.value = JSON.parse(
        res.data_assessment_result.desc.description
      );
      data_desc.value.push([
        Math.floor(Math.max(...res.data_assessment_result.percentage)),
        res.data_assessment_result.desc,
      ]);
    }

    if (
      res.data_cpm_result.desc &&
      !["SMK", "SMA", "SMP", "Perguruan Tinggi"].includes(schoolType.value)
    ) {
      data_desc.value.push([
        Math.floor(Math.max(...res.data_cpm_result.percentage)),
        res.data_cpm_result.desc,
      ]);
    }

    if (res.data_iq_result.desc && !["TK", "SD"].includes(schoolType.value)) {
      data_desc.value.push([
        Math.floor(Math.max(...res.data_iq_result.percentage)),
        res.data_iq_result.desc,
      ]);
    }

    console.log("data_desc", data_desc.value);
  } catch (error) {
    console.log(error);
    Swal.fire({
      icon: "error",
      title: "Error",
      text: "Terjadi error saat mengambil data statistik dashboard.",
...
watch(selectedInstitutionId, (newVal, oldVal) => {
  ...
  if (newVal) {
    fetchDataStatusAPI(); // Ambil data statistik baru
    getSiswa();
  }
});

onMounted(async () => {
  await getPlacementData(); // Ambil placement & set default institusi
  await fetchDataStatusAPI(); // Baru ambil data statistik berdasarkan institusi yang sudah diset
  await getSiswa();
});
