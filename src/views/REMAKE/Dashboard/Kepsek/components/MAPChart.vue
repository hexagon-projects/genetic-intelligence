<template>
    <div class="p-8 rounded-lg bg-white font-sora">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Motivation for Academic & Professional</h2>

        <div class="space-y-3 mb-8">
            <div v-for="(item, index) in motivationData" :key="item.label" class="relative group cursor-pointer"
                ...
                <div class="flex items-center space-x-3">
                    <div class="flex-1">
                        <div class="relative bg-gray-200 rounded-full h-12 overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-1000 ease-out flex items-center px-4"
                                :class="[
        ...
        <div class="bg-gray-200 rounded-xl p-6">
            <div class="flex flex-col md:flex-row items-end gap-6 md:gap-8">
                <div class="w-fit bg-white p-2 rounded-lg space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <div class="text-gray-600 text-sm">
                                Orang Memiliki Motivasi<br>Sangat Tinggi
                            </div>
                        </div>
                    </div>
                </div>

                <div class="w-fit flex items-center justify-between gap-2 md:gap-4">
                    <span class="text-sm text-gray-500">Rendah</span>
                    <div class="flex space-x-1">
                        <div v-for="(color, index) in gradientColors" :key="index" class="w-4 h-4 md:w-6 md:h-6 rounded-sm"
...
const motivationData = computed<MotivationLevel[]>(() => {
    if (!props.mapStatistik) return []

    const totalCustomers = props.mapStatistik.reduce((sum, item) => sum + item.total_customer, 0)
    const colors = ['#E26F64', '#E88C83', '#EEA9A2', '#F3C5C1']

    return props.mapStatistik.map((stat, index) => ({
        label: stat.kategori,
        value: stat.total_customer,
        percentage: totalCustomers > 0 ? Math.round((stat.total_customer / totalCustomers) * 100) : 0,
        color: colors[index] || '#ea580c'
    }))
})

const extremeMotivation = computed(() => {
    return props.mapStatistik?.find(stat =>
        stat.kategori.toLowerCase().includes('sangat tinggi')
    ) || { total_customer: 0 }
})

const extremePercentage = computed(() => {
    const totalCustomers = props.mapStatistik?.reduce((sum, item) => sum + item.total_customer, 0) || 0
    return totalCustomers > 0 ? Math.round((extremeMotivation.value.total_customer / totalCustomers) * 100) : 0
})

const gradientColors = ['#E26F64', '#E88C83', '#EEA9A2', '#F3C5C1']
...
watch(() => props.mapStatistik, () => {
    if (props.mapStatistik) {
        setTimeout(() => {
            animateNumber(extremeCountRef.value, extremeMotivation.value.total_customer)
        }, 800)
    }
}, { immediate: true })
