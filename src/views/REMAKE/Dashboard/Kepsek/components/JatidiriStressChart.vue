<template>
    <div class="w-full bg-white rounded-2xl p-6">
        <h2 class="text-xl md:text-2xl font-bold text-gray-900 mb-4">
            Statistik Jatidiri Kendali Stress
        </h2>
            ...
            <div class="relative">
                <div class="relative w-80 h-80">
                    ...
                    <div v-if="!isLoading" class="absolute inset-0 flex flex-col items-center justify-center center-text-animation">
                        ...
                        <div class="text-gray-500 text-sm">Stress</div>
                    </div>
                </div>
...
const stressData = computed<StressDataItem[]>(() => {
    if (!props.data?.statistics) {
        return []
    }
    
    return props.data.statistics.map((stat) => ({
        label: stat.name,
        value: stat.count,
        color: colorMap[stat.name] || '#dc2626',
        percentage: stat.percent
    }))
})

const totalValue = computed(() =>
    stressData.value.reduce((sum, item) => sum + item.value, 0)
)
...
const chartSegments = computed(() => {
    ...
    return stressData.value.map((item) => {
