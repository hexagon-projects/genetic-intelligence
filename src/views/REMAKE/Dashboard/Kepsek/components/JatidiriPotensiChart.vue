<template>
    <div class="w-full p-6 bg-white rounded-lg flex flex-col justify-between gap-6">
        ...
        <div class="w-full flex flex-col gap-4 md:gap-6 bg-gray-200 p-4 rounded-lg">
            ...
            <div class="w-full flex items-end gap-4">
                ...
                <p class="text-sm text-gray-600">
                    Potensi komunikasi memiliki skor tertinggi dengan 90 poin, menunjukkan kemampuan interaksi yang sangat baik. Sementara potensi teknis memiliki skor terendah dengan 65 poin, mengindikasikan area yang perlu pengembangan lebih lanjut.
                </p>
            </div>
        </div>
        ...
        <div v-if="tooltip.show"
            ...
            <div class="text-gray-300">Value: {{ tooltip.data?.value }}</div>
        </div>
    </div>
</template>
...
const chartData: ChartItem[] = [
    { label: 'Kepemimpinan', value: 85 },
    { label: 'Kreativitas', value: 72 },
    { label: 'Analitis', value: 68 },
    { label: 'Komunikasi', value: 90 },
    { label: 'Kerjasama', value: 78 },
    { label: 'Teknis', value: 65 }
]
...
const maxValue = computed(() => Math.max(...chartData.map(item => item.value)))
const totalValue = computed(() => chartData.reduce((sum, item) => sum + item.value, 0))
