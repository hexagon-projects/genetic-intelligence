            ...
            <div
                ...
                <div v-if="!isLoading" class="w-full flex justify-between items-center gap-4">
                    <button @click="prevQuestion"
                        ...
                        {{ currentQuestionIndex <= 0 ? 'Batal' : 'Kembali' }} </button>
                            <button @click="currentQuestionIndex < 0 ? startQuestionnaire() : nextQuestion()"
                                class="py-3 text-sm md:text-base w-full border border-gray-300 rounded-xl cursor-pointer text-white font-medium"
                                :class="isOptionSelected ? 'bg-primary' : 'bg-gray-400'" :disabled="!isOptionSelected">
                                {{ currentQuestionIndex < 0 ? 'Mulai' : 'Selanjutnya' }} </button>
                </div>
            </div>
        </div>
    </Layout>
