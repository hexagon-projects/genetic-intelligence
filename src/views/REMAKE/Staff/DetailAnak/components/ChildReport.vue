            ...
            <div class="flex flex-wrap sm:flex-nowrap overflow-x-auto scrollbar-hide">
                <div class="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-2 cursor-pointer transition-all duration-300 whitespace-nowrap min-w-fit"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="w-3 h-3 sm:w-4 sm:h-4 md:w-[17px] md:h-4 flex-shrink-0" viewBox="0 0 17 16" fill="none">
                        <path
                    ...
                    <p class="text-[10px] sm:text-xs"
                        :class="activeResultTab === 'kesimpulan' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">
                        Kesimpulan
                    </p>
                </div>
                ...
                <div class="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-2 cursor-pointer transition-all duration-300 whitespace-nowrap min-w-fit"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="w-3 h-3 sm:w-4 sm:h-4 md:w-[13px] md:h-3 flex-shrink-0" viewBox="0 0 13 12" fill="none">
            ...
            <transition name="fade" mode="out-in">
                <div :key="activeResultTab"
                    ...
                    <template v-if="activeResultTab === 'kesimpulan' && teacherConclusion">
                        <p class="text-xs sm:text-sm">{{ teacherConclusion.content }}</p>
                        <div
                            class="py-1 sm:py-1.5 px-3 sm:px-4 rounded-full bg-primary w-fit text-xs sm:text-sm text-white">
                            Skor {{ teacherConclusion.score }}
                        </div>
                    </template>

                    <!-- Tampilan normal untuk tab lainnya -->
                    <template v-else>
            ...
            <div class="flex flex-wrap items-center gap-2 md:gap-4">
                <div v-for="(doc, index) in getReportDocumentations" :key="index">
                    <img :src="getImageUrl(doc.file)" :alt="doc.title"
                        class="w-24 h-24 md:w-32 md:h-32 rounded-xl object-contain cursor-pointer"
                        @click="props.openImageModal(doc)">
                </div>
            </div>
        </div>
