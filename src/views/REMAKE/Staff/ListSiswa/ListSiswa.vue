    ...
    <section class="bg-white py-[66px] px-[20px] md:px-[60px]">
      <div class="flex flex-col gap-[36px]">
        <div class="flex justify-between items-center mb-5">
          ...
          <button class="text-white rounded-md py-2 px-3 bg-[#12B76A] font-medium flex items-center gap-3"
            @click="exportData" :disabled="exportLoading">
      ...
      <div class="flex flex-col gap-[13px] mb-3">
        <div class="flex flex-wrap justify-between items-center gap-4">
          <div class="flex items-center gap-2">
            <button @click="showFilter = !showFilter" :class="{ 'bg-[#f6f6f9]': showFilter, 'bg-white': !showFilter }"
            ...
            <div class="text-[#32324D] font-['Roboto'] leading-loose relative">
              ...
              <svg width="8" height="4" viewBox="0 0 8 4" fill="none"
                class="absolute right-3 top-1/2 transform -translate-y-1/2" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="flex items-center gap-4">
            <div class="text-[#32324D] font-['Roboto'] leading-loose flex flex-row">
              <input v-model="searchQuery" type="text" placeholder="Cari data pengguna"
        ...
        <div v-if="showFilter" class="p-4 bg-white rounded-xl border border-[#cfd4dc]">
          <div class="flex flex-wrap gap-4">
            <FilterItem label="Kelas" v-model="kelas" FilterType="kelas" :options="kelasOptions"
              @reset="resetFilter('kelas')" />

            <FilterItem label="Kecerdasan Dominan" v-model="tipeKecerdasan" :options="tipeKecerdasanOptions"
              @reset="resetFilter('tipeKecerdasan')" />

            <FilterItem label="Gaya Belajar" v-model="gayaBelajar" :options="gayaBelajarOptions"
              @reset="resetFilter('gayaBelajar')" />

            <FilterItem label="Grade IQ" v-model="skorIq" :options="skorIqOptions" @reset="resetFilter('skorIq')" />
          </div>
        </div>
      </div>
      ...
      <!-- Overview Data Siswa -->
      <div class="space-y-4">
        <div class="bg-transparent border border-gray-300 p-4 flex flex-col rounded-xl">
                  ...
                  <td class="py-3 px-4">
                    <button @click="detailSiswa(siswa.id)"
                      ...
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M1.66669 8.33335L8.33335 1.66669M8.33335 1.66669H1.66669M8.33335 1.66669V8.33335"
                          stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                      </svg>
                    </button>
                  </td>
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <FilterItem label="10" v-model="perPage" :options="perPageOptions" @reset="resetFilter('perPage')" />
          ...
          <div class="flex items-center gap-3">
            <!-- Button Previous -->
            <button :class="{ 'opacity-40 cursor-not-allowed': currentPage == 1 }" class="flex items-center gap-[6px]"
            ...
            <button v-for="page in pagesToShow" :key="page" @click="goToPage(page)"
              class="font-roboto py-1 px-3 hover:text-[#271FE0]" :class="{
            ...
            <!-- Button Next -->
            <button :class="{ 'opacity-40 cursor-not-allowed': currentPage == lastPage }"
...
const exportData = async () => {
  try {
    ...
    const institutionId = kepsekData.value.staff.institution_id;
    
    // Request ke API untuk mendapatkan file PDF
    ...
    // link.download = `institution_data_${institutionId}.pdf`;
    // document.body.appendChild(link);
    // link.click();
    // document.body.removeChild(link);

    window.location.href = `https://api.jatidiri.app/api/institutions/${institutionId}`
    
    Swal.fire({
      icon: "success",
      title: "Export Berhasil",
      text: "Data berhasil diunduh dalam format PDF",
    ...
    Swal.fire({
      icon: "error",
      title: "Export Gagal",
      text: "Gagal mengunduh data institusi",
      showConfirmButton: false,
      timer: 2000,
    });
  } finally {
