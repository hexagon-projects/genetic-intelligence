...
const validateBiodata = () => {
  ...
  if (missingFields.length > 0) {
    Swal.fire({
      ...
      html: `Silakan lengkapi biodata Anda terlebih dahulu di halaman profile. <br>Field yang harus diisi: ${missingFields.join(', ')}`,
  ...
  if (userData.birth_date) {
    ...
    if (ageInMonths > 36) { // 36 bulan = 3 tahun
      Swal.fire({
        ...
        text: 'Kuisioner ini hanya untuk anak usia 0-3 tahun. Anak Anda sudah berusia di atas 3 tahun.',
...
const fetchQuestions = async () => {
  try {
    ...
    const response = await initAPI('get', 'kids/questions', null, token);
...
const submitAnswers = async () => {
  try {
    ...
    const answerTypes = ['kognisi', 'psikomotorik', 'emosi', 'relasi', 'mandiri'];
    
    answerTypes.forEach(type => {
      if (groupedAnswers[type].length < requiredLength) {
        while (groupedAnswers[type].length < requiredLength) {
          groupedAnswers[type].push(2);
        }
      } else if (groupedAnswers[type].length > requiredLength) {
        groupedAnswers[type] = groupedAnswers[type].slice(0, requiredLength);
      }
    });
    ...
    const token = Cookies.get('token');
    const response = await initAPI('post', 'customers/kid', payload, token);
    
    if (response.data && (response.data.message === "Jawaban Berhasil Direkam." || response.data.success)) {
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
    ...
    Swal.fire({
      icon: 'error',
      title: 'Gagal Mengirim Jawaban',
      text: error.message || 'Terjadi kesalahan saat mengirim jawaban. Silakan coba lagi.',
      showConfirmButton: true
    });
  } finally {
    isLoading.value = false;
