...
const assessmentResults = ref({
  psikomotor: {
    title: "Psikomotor",
    content: "Ananda A secara umum menunjukkan kemampuan motorik yang baik. Ananda A dalam menggunakan pensil dalam menulis, masih digenggam belum menggunakan tiga jari utama, namun Ia dapat menggoreskan huruf-huruf dengan tekanan tangan yang kebanyakan stabil. Ada saat dimana tester meminta Ananda A untuk menuliskan namanya namun Ia belum bisa, kemudian tester beri contoh untuk mencoba menuliskannya sendiri. Dalam situasi tersebut, Ananda A menunjukkan koordinasi mata tangan yang baik dalam menyalin tulisan. Dalam aktivitas menggambar, gerakan tangan terlihat baik terkendali, meskipun hasil gambarnya belum simetris. Ananda A mampu melempar dan menangkap bola dengan koordinasi tubuh yang baik. Ananda A dapat mengarahkan lemparan bola ke sasaran dengan cukup tepat dan cukup mampu menyesuaikan gerakan tangan saat bola dilemparkan kepadanya. Saat melintasi titian Ananda A menunjukkan keseimbangan yang baik. Ananda A mampu berjalan lurus menyusuri titian dengan langkah stabil tanpa terjatuh atau keluar dari batas. Gerakan kaki dilakukan dengan hati-hati namun mantap, serta postur tubuh tetap tegak selama berjalan di atas titian dengan koordinasi mata melihat ke lintasan titian.",
    score: 4,
    note: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad iusto deleniti nostrum maiores illo alias placeat voluptate! Repellat, in ut.",
    report: "laporan_psikomotor.pdf"
  },
  kognisi: {
    title: "Kognisi",
    content: "Ananda A menunjukkan perkembangan kognitif yang baik untuk usianya. Ia mampu mengenali warna dasar, bentuk geometris sederhana, dan menghitung hingga angka 10 dengan bantuan. Ananda A juga menunjukkan minat dalam kegiatan bercerita dan dapat mengingat alur cerita sederhana. Kemampuan pemecahan masalahnya masih dalam tahap awal namun menunjukkan potensi yang baik.",
    score: 3,
    note: "Perlu lebih banyak stimulasi untuk meningkatkan kemampuan berpikir logis dan pemecahan masalah.",
    report: "laporan_kognisi.pdf"
  },
  emosi: {
    title: "Emosi",
    content: "Ananda A memiliki kecerdasan emosional yang berkembang baik. Ia dapat mengenali dan mengekspresikan perasaan dasar seperti senang, sedih, dan marah dengan tepat. Ananda A juga mulai menunjukkan empati terhadap teman-temannya dan dapat mengelola frustrasi dengan bimbingan orang dewasa. Namun, masih perlu bimbingan dalam mengungkapkan perasaan yang lebih kompleks.",
    score: 4,
    note: "Anak perlu belajar mengungkapkan perasaan dengan kata-kata daripada tindakan fisik.",
    report: "laporan_emosi.pdf"
  },
  relasiSosial: {
    title: "Relasi Sosial",
    content: "Ananda A adalah anak yang ramah dan mudah bergaul dengan teman sebayanya. Ia aktif dalam kegiatan kelompok dan dapat berbagi mainan dengan bimbingan. Ananda A memahami konsep bergantian dan dapat mengikuti aturan permainan sederhana. Ia menunjukkan minat dalam berinteraksi dengan orang dewasa dan anak-anak lain.",
    score: 5,
    note: "Perlu terus didorong untuk berinteraksi dalam kelompok yang lebih besar.",
    report: "laporan_relasi_sosial.pdf"
  },
  kemandirian: {
    title: "Kemandirian",
    content: "Ananda A menunjukkan kemandirian yang baik dalam kegiatan sehari-hari. Ia dapat makan sendiri, memakai sepatu, dan merapikan mainannya setelah bermain dengan sedikit bimbingan. Ananda A juga mulai menunjukkan inisiatif dalam menyelesaikan tugas-tugas sederhana dan dapat mengikuti rutinitas dengan baik.",
    score: 4,
    note: "Perlu terus dilatih untuk meningkatkan kemandirian dalam kegiatan self-care.",
    report: "laporan_kemandirian.pdf"
  }
});

const activeTab = ref('psikomotor');
...
const downloadPDF = (filename) => {
  console.log(`Mengunduh file: ${filename}`);
  alert(`File ${filename} akan diunduh.`);
};
</script>

<template>
    <Layout>
        <div class="p-4 min-h-screen bg-[#F6F6F6] font-sora flex justify-center items-center">
            <div class="w-full md:max-w-[90%] lg:max-w-[70%] xl:max-w-[50%] mx-auto bg-white p-6 rounded-3xl space-y-4 md:space-y-6 shadow-md shadow-black/5">
                <!-- Tab -->
                <div class="flex flex-col">
                    <div class="flex">
                        <!-- Tab Psikomotor -->
                        <div 
                            class="flex items-center gap-2 px-4 py-2 cursor-pointer transition-all duration-300"
                            :class="activeTab === 'psikomotor' ? 'bg-[#f5f5f5] rounded-t-xl' : ''"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16"
                                ...
                                <path
                                    ...
                                    :stroke="activeTab === 'psikomotor' ? '#6464FA' : '#8E8E8E'" />
                                <path
                            ...
                            <p class="text-xs" :class="activeTab === 'psikomotor' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">Psikomotor</p>
                        </div>

                        <!-- Tab Kognisi -->
                        <div 
                            class="flex items-center gap-2 px-4 py-2 cursor-pointer transition-all duration-300"
                            :class="activeTab === 'kognisi' ? 'bg-[#f5f5f5] rounded-t-xl' : ''"
                            ...
                            <p class="text-xs" :class="activeTab === 'kognisi' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">Kognisi</p>
                        </div>

                        <!-- Tab Emosi -->
                        <div 
                            class="flex items-center gap-2 px-4 py-2 cursor-pointer transition-all duration-300"
                            :class="activeTab === 'emosi' ? 'bg-[#f5f5f5] rounded-t-xl' : ''"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12"
                                ...
                                <path
                                    ...
                                    :stroke="activeTab === 'emosi' ? '#6464FA' : '#8E8E8E'" />
                                <path
                                    d="M6.5 3.5L7.2125 4.7875L8.5 5.5L7.2125 6.2125L6.5 7.5L5.7875 6.2125L4.5 5.5L5.7875 4.7875L6.5 3.5Z"
                                    :stroke="activeTab === 'emosi' ? '#6464FA' : '#8E8E8E'" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>

                            <p class="text-xs" :class="activeTab === 'emosi' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">Emosi</p>
                        </div>

                        <!-- Tab Relasi Sosial -->
                        <div 
                            class="flex items-center gap-2 px-4 py-2 cursor-pointer transition-all duration-300"
                            :class="activeTab === 'relasiSosial' ? 'bg-[#f5f5f5] rounded-t-xl' : ''"
                            @click="changeTab('relasiSosial')">
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12"
                                fill="none">
                                <path
                                    d="M2 1.8C2 1.72044 2.03161 1.64413 2.08787 1.58787C2.14413 1.53161 2.22044 1.5 2.3 1.5H10.7C10.7796 1.5 10.8559 1.53161 10.9121 1.58787C10.9684 1.64413 11 1.72044 11 1.8V8.7C11 8.7394 10.9922 8.77841 10.9772 8.81481C10.9621 8.8512 10.94 8.88427 10.9121 8.91213C10.8843 8.93999 10.8512 8.96209 10.8148 8.97716C10.7784 8.99224 10.7394 9 10.7 9H8.63C8.58897 8.99994 8.54837 9.0083 8.5107 9.02455C8.47304 9.0408 8.4391 9.06461 8.411 9.0945L6.7185 10.893C6.69045 10.9228 6.65659 10.9466 6.61902 10.9628C6.58144 10.9791 6.54094 10.9874 6.5 10.9874C6.45906 10.9874 6.41856 10.9791 6.38098 10.9628C6.34341 10.9466 6.30955 10.9228 6.2815 10.893L4.589 9.0945C4.5609 9.06461 4.52696 9.0408 4.4893 9.02455C4.45163 9.0083 4.41103 8.99994 4.37 9H2.3C2.22044 9 2.14413 8.96839 2.08787 8.91213C2.03161 8.85587 2 8.77956 2 8.7V1.8Z"
                                    :stroke="activeTab === 'relasiSosial' ? '#6464FA' : '#8E8E8E'" />
                                <path
                                    d="M6.5 3.5L7.2125 4.7875L8.5 5.5L7.2125 6.2125L6.5 7.5L5.7875 6.2125L4.5 5.5L5.7875 4.7875L6.5 3.5Z"
                                    :stroke="activeTab === 'relasiSosial' ? '#6464FA' : '#8E8E8E'" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>

                            <p class="text-xs" :class="activeTab === 'relasiSosial' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">Relasi Sosial</p>
                        </div>

                        <!-- Tab Kemandirian -->
                        <div 
                            class="flex items-center gap-2 px-4 py-2 cursor-pointer transition-all duration-300"
                            :class="activeTab === 'kemandirian' ? 'bg-[#f5f5f5] rounded-t-xl' : ''"
                            @click="changeTab('kemandirian')">
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12"
                                fill="none">
                                <path
                                    d="M2 1.8C2 1.72044 2.03161 1.64413 2.08787 1.58787C2.14413 1.53161 2.22044 1.5 2.3 1.5H10.7C10.7796 1.5 10.8559 1.53161 10.9121 1.58787C10.9684 1.64413 11 1.72044 11 1.8V8.7C11 8.7394 10.9922 8.77841 10.9772 8.81481C10.9621 8.8512 10.94 8.88427 10.9121 8.91213C10.8843 8.93999 10.8512 8.96209 10.8148 8.97716C10.7784 8.99224 10.7394 9 10.7 9H8.63C8.58897 8.99994 8.54837 9.0083 8.5107 9.02455C8.47304 9.0408 8.4391 9.06461 8.411 9.0945L6.7185 10.893C6.69045 10.9228 6.65659 10.9466 6.61902 10.9628C6.58144 10.9791 6.54094 10.9874 6.5 10.9874C6.45906 10.9874 6.41856 10.9791 6.38098 10.9628C6.34341 10.9466 6.30955 10.9228 6.2815 10.893L4.589 9.0945C4.5609 9.06461 4.52696 9.0408 4.4893 9.02455C4.45163 9.0083 4.41103 8.99994 4.37 9H2.3C2.22044 9 2.14413 8.96839 2.08787 8.91213C2.03161 8.85587 2 8.77956 2 8.7V1.8Z"
                                    :stroke="activeTab === 'kemandirian' ? '#6464FA' : '#8E8E8E'" />
                                <path
                                    d="M6.5 3.5L7.2125 4.7875L8.5 5.5L7.2125 6.2125L6.5 7.5L5.7875 6.2125L4.5 5.5L5.7875 4.7875L6.5 3.5Z"
                                    :stroke="activeTab === 'kemandirian' ? '#6464FA' : '#8E8E8E'" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>

                            <p class="text-xs" :class="activeTab === 'kemandirian' ? 'text-[#6464FA]' : 'text-[#8E8E8E]'">Kemandirian</p>
                        </div>
                    </div>

                    <!-- Isi Konten Tab Aktif dengan animasi -->
                    <transition name="fade" mode="out-in">
                        <div :key="activeTab" class="p-4 bg-[#F5F5F5] rounded-b-xl rounded-tr-xl space-y-4">
                            <p class="text-xs">{{ assessmentResults[activeTab].content }}</p>

                            <div class="py-1 px-4 rounded-full bg-primary w-fit text-xs text-white">
                                Skor {{ assessmentResults[activeTab].score }}
                            </div>
                        </div>
                    </transition>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <!-- Isi Konten Tab Aktif dengan animasi -->
                    <transition name="fade" mode="out-in">
                        <div :key="activeTab" class="w-full bg-[#F5F5F5] p-4 rounded-xl">
                            <p class="text-xs">{{ assessmentResults[activeTab].note }}</p>
                        </div>
                    </transition>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <!-- Isi Konten Tab Aktif dengan animasi -->
                    <transition name="fade" mode="out-in">
                        <div :key="activeTab" class="w-full bg-[#F5F5F5] p-4 rounded-xl flex justify-between items-center">
                            <!-- Download Laporan PDF -->
                            <a class="text-xs text-primary">{{ assessmentResults[activeTab].report }}</a>

                            <button @click="downloadPDF(assessmentResults[activeTab].report)" class="cursor-pointer">
