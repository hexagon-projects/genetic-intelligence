<template>
    ...
    <Layout v-if="!loading">
        ...
        <BelumTest v-if="!isTested" routeUrl="user.views.test_kebahagiaan" message="Test Jatidiri Stres" 
        subMessage="Perceived Stress Scale (PSS) adalah alat tes psikologis yang dirancang untuk mengukur tingkat stres individu berdasarkan perasaan dan pikiran Anda selama sebulan terakhir."/>
...
import 'sweetalert2/dist/sweetalert2.css';
  ...
  try {
      ...
      const response = await initAPI('get', `customers/stres?customer_id=${userId}`, null, token)
      // console.log(`data iaa`, response.data)

      console.log(response)
      isTested.value = response.data.data ? true : false
      hasilStres.value = response.data.data ? response.data.data[0].name : null
      descStres.value = response.data.data ? response.data.data[0].category : null
