<template>
    ...
    <Layout v-if="!loading">
        ...
        <BelumTest v-if="!isTested" routeUrl="user.views.test_kebahagiaan" message="Test Jatidiri Kesehatan Mental" 
        subMessage="Kesehatan Mental adalah alat tes psikologis yang dirancang untuk mengukur tingkat kesehatan mental individu berdasarkan kondisi dan pikiran Anda selama seminggu terakhir."/>
                          ...
                          <div class="flex flex-col md:flex-row justify-center md:justify-between items-center gap-6 leading-[24px] ">
                              ...
                              <span class=" text-[#FFF] text-base font-normal font-['Roboto'] leading-6 mt-2 not-italic">
                                  <!-- {{ formattedCategoryDesc }} -->
                                    <div class="[&_a]:text-blue-600 [&_a]:underline [&_a]:font-medium [&_a:hover]:text-blue-800
        [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:my-4
        [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:my-4
        [&_li]:my-2" v-html="descMental"></div>
                              </span>
                              </div>
                          </div>
                      </div> 
...
import 'sweetalert2/dist/sweetalert2.css';
  ...
  const hasilMental = ref('')
  const descMental = ref('')
  const videoResult = ref('')

  const getTikData = async(userId) => {
  try {
      const token = Cookies.get('token')
      const response = await initAPI('get', `customers/mental?customer_id=${userId}`, null, token)
      // console.log(`data iaa`, response.data)

      console.log(response)
      isTested.value = response.data.data ? true : false
      hasilMental.value = response.data.data ? response.data.data[0].name : null
      descMental.value = response.data.data ? response.data.data[0].category : null
