...
<div v-if="loading" class="preloader-overlay">
    ...
    <Layout v-if="!loading">
        <div class="mx-0 lg:mx-[40px] mb-3 h-5 p-7 justify-center items-center gap-2 inline-flex">
            ...
            <div class="opacity-75 text-black text-sm font-normal font-roboto leading-tight">Jatidiri Kesehatan Mental</div>
        </div>

        <SelesaiTest v-if="isTested" routeUrl="user.views.hasil_jatidiri_mental" message="Test Kesehatan Mental Selesai!"
        ...
        <section class="bg-white pb-[34px] mb-10" v-if="!isTested">
            ...
            <div class="flex flex-col justify-center items-center gap-[24px] mb-10">
                <div class="flex flex-col items-center gap-[56px]">
                    <div class="mx-[30px] md:mx-[60px] h-auto flex-col justify-start items-start gap-4 inline-flex">
                        <span class="self-stretch text-center text-black text-xl md:text-3xl font-semibold font-['Sora'] leading-9">Tes Kesehatan Mental</span>
                        <span class="self-stretch text-center text-black text-sm md:text-base font-normal font-['Roboto'] leading-normal">Pilihlah Pilih jawaban yang paling sesuai dengan kondisi anda saat ini dengan memilih jawaban pada salah satu pilihan berikut.</span>
                        <ol class="self-stretch text-black text-sm md:text-base font-normal font-['Roboto'] leading-normal list-decimal list-inside" start="0">
                            <li>Sangat Tidak Sehat / Tidak Sama Sekali</li>
                            <li>Kurang Sehat / Jarang</li>
                            <li>Cukup Sehat / Kadang-kadang</li>
                            <li>Sehat / Sering</li>
                            <li>Sangat Sehat / Selalu</li>
                        </ol>
                    </div>

                    <!-- pertanyaan -->
                    <div id="soal" v-if="dataPertanyaan" class="mx-[30px] md:mx-[60px] w-full md:w-3/4 h-auto p-9 bg-white md:rounded-3xl shadow flex flex-col gap-9">
...
const subMessage = `Kerja yang bagus! Kamu telah menyelesaikan Tes <span class="font-bold">Kesehatan Mental</span>. Mari lihat hasilnya dan temukan lebih banyak tentang potensi diri Kamu!`
...
const getUserData = async() => {
    try {
        ...
        isTested.value = userData.data.customer.customers_mental == null ? false : true
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const token = Cookies.get('token');
                    const response = await initAPI('post', 'customers/mental', payload, token);
...
const getDataPertanyaan = async(page = 1) => {
    ...
    if(token){
        try {
            const response = await initAPI('get', `mental/questions`, null, token)
