            ...
            <div class="flex flex-col justify-center items-center gap-[24px] mb-10">
                <div class="flex flex-col items-center gap-[56px]">
                    <div class="mx-[30px] md:mx-[60px] h-auto flex-col justify-start items-start gap-4 inline-flex">
                        <span class="self-stretch text-center text-black text-xl md:text-3xl font-semibold font-['Sora'] leading-9">Tes Index Kebahagiaan</span>
                        ...
                        <ol class="self-stretch text-black text-sm md:text-base font-normal font-['Roboto'] leading-normal">
                            <li>1. Sangat Jarang (Hampir Tidak Pernah)</li>
                            <li>2. Jarang (Terkadang Terjadi)</li>
                            <li>3. Kadang - kadang (Sering terjadi, tapi tidak selalu)</li>
                            <li>4. Sering (Biasa Terjadi)</li>
                            <li>5. Selalu (Hampir Selalu Terjadi)</li>
                        </ol>
                    </div>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const token = Cookies.get('token');
                    const response = await initAPI('post', 'customers/tik', payload, token);
...
const getDataPertanyaan = async(page = 1) => {
    ...
    if(token){
        try {
            const response = await initAPI('get', `tik/questions`, null, token)
