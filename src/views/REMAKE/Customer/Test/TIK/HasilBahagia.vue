<template>
    ...
    <Layout v-if="!loading">
        ...
        <BelumTest v-if="!isTested" routeUrl="user.views.test_kebahagiaan" message="Test Jatidiri Bahagia" 
        subMessage="Tes ini dirancang untuk mengukur tingkat kebahagiaan individu berbagai berdasarkan berbagai aspek kehidupan dan keseimbangan hidup"/>
...
import 'sweetalert2/dist/sweetalert2.css';
  ...
  try {
      ...
      const response = await initAPI('get', `customers/tik?customer_id=${userId}`, null, token)
      // console.log(`data iaa`, response.data)

      console.log(response)
      isTested.value = response.data.data ? true : false
      hasilTik.value = response.data.data ? response.data.data[0].name : null
      descTik.value = response.data.data ? response.data.data[0].category : null
