    ...
    <Layout>
        ...
        <section v-if="!isTested" class="pb-[34px] w-full bg-white">
            ...
            <div v-if="!isInstruksi" class="mb-[48px] w-full flex justify-center items-center">
                <SoalTest :customerId="customerId" @refreshUserData="getUserData"/>
            </div>
        </section>
    </Layout>
</template>
...
import Instruksi from '@/components/REMAKE/Test/IQ/Instruksi.vue';
...
const getUserData = async() => {
    try {
        ...
        isTested.value = userData.data.customer.customers_iq == null ? false : true
    } catch (error) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
