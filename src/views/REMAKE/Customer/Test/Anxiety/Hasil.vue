<template>
    ...
    <Layout v-if="!loading">
        ...
        <BelumTest v-if="!isTested" routeUrl="user.views.test_kebahagiaan" message="Test Jatidiri Anxiety" 
        subMessage="Self-Rating Scale for Anxiety (SRA-Scale) adalah alat tes psikologis yang dirancang untuk mengukur tingkat kecemasan individu berdasarkan penilaian diri (self-rating)."/>
        ...
        [&_li]:my-2" v-html="descAnxiety"></div>
                              </span>
                              </div>
                          </div>
                      </div> 
...
import 'sweetalert2/dist/sweetalert2.css';
  ...
  const hasilAnxiety = ref('')
  const descAnxiety = ref('')
  const videoResult = ref('')

  const getTikData = async(userId) => {
  try {
      const token = Cookies.get('token')
      const response = await initAPI('get', `customers/anxiety?customer_id=${userId}`, null, token)
      // console.log(`data iaa`, response.data)

      console.log(response)
      isTested.value = response.data.data ? true : false
      hasilAnxiety.value = response.data.data ? response.data.data[0].name : null
      descAnxiety.value = response.data.data ? response.data.data[0].category : null
