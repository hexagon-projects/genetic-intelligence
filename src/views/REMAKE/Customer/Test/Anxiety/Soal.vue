...
<div v-if="loading" class="preloader-overlay">
    ...
    <Layout v-if="!loading">
        <div class="mx-0 lg:mx-[40px] mb-3 h-5 p-7 justify-center items-center gap-2 inline-flex">
            ...
            <div class="opacity-75 text-black text-sm font-normal font-roboto leading-tight">Jatidiri Anxiety</div>
        </div>
            ...
            <div class="flex flex-col justify-center items-center gap-[24px] mb-10">
                <div class="flex flex-col items-center gap-[56px]">
                    <div class="mx-[30px] md:mx-[60px] h-auto flex-col justify-start items-start gap-4 inline-flex">
                        <span class="self-stretch text-center text-black text-xl md:text-3xl font-semibold font-['Sora'] leading-9">Tes Anxiety</span>
                        ...
                        <ol class="self-stretch text-black text-sm md:text-base font-normal font-['Roboto'] leading-normal list-decimal list-inside" start="0">
                            <li>Tidak Pernah (Never)</li>
                            <li>Kadang - kadang (Sometimes)</li>
                            <li>Setengah Waktu (Half Time)</li>
                            <li>Sering (FREQUENTLY)</li>
                            <li>Selalu (Always)</li>
                        </ol>
                    </div>
...
const getUserData = async() => {
    try {
        ...
        isTested.value = userData.data.customer.customers_anxiety == null ? false : true
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    const token = Cookies.get('token');
                    const response = await initAPI('post', 'customers/anxiety', payload, token);
...
const getDataPertanyaan = async(page = 1) => {
    ...
    if(token){
        try {
            const response = await initAPI('get', `anxiety/questions`, null, token)
