...
const STORAGE_KEY = 'bri_test_progress';
const ANSWERS_KEY = 'bri_test_answers';
...
const checkTestEligibility = (history: any[]) => {
    ...
    const lastTestDate = new Date(latestTest.assement.created_at);
    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.length > 0) {
            ...
            testHistory.value.sort((a, b) =>
                new Date(b.bri.created_at).getTime() - new Date(a.bri.created_at).getTime()
            );
...
const fetchQuestions = async () => {
    try {
        ...
        const response = await initAPI('GET', 'bri/questions', null, token);
...
const formatAnswersForAPI = () => {
    const exhaustionAnswers: number[] = [];
    const depersonalizationAnswers: number[] = [];
    const resilienceAnswers: number[] = [];
    ...
    questions.value.forEach(question => {
        ...
        switch (question.type) {
            case 'Exhaustion':
                exhaustionAnswers.push(value);
                break;
            case 'Depersonalization':
                depersonalizationAnswers.push(value);
                break;
            case 'Resilience':
                resilienceAnswers.push(value);
                break;
        }
    });
    ...
    return {
        ...
        exhaustion: exhaustionAnswers,
        depersonalization: depersonalizationAnswers,
...
const submitAnswers = async () => {
    try {
        ...
        const response = await initAPI('POST', 'customers/bri', payload, token);

        if (response.data && response.data.message === 'Jawaban Berhasil Direkam.') {
            localStorage.removeItem(STORAGE_KEY);
            localStorage.removeItem(ANSWERS_KEY);
            showSuccessModal.value = true;
            localStorage.setItem('bri_id', response?.data.result.id)
        }
    } catch (err) {
...
<template>
    <div class="w-full bg-[#F8FBFE] font-sora">
        <div class="h-screen p-4 space-y-6 md:max-w-[50%] lg:max-w-[30%] xl:max-w-[25%] mx-auto relative">
            <div v-if="testEligibilityChecked && !canTakeTest"
                ...
                <div class="w-full md:max-w-lg">
                    ...
                    <NewButton @click="navigateToBRI" text="Kembali ke Halaman BRI" class="w-full font-semibold"
                        text-size="text-sm" />
                </div>
            </div>
            ...
            <div v-else-if="testEligibilityChecked && canTakeTest" class="w-full h-full">
                <div class="w-full text-center flex justify-center items-center gap-2 mb-6 md:mb-8">
                    ...
                    <h1 class="text-sm font-bold">Tes Jatidiri Tangguh - BRI</h1>
                </div>
        ...
        <CustomModal :show="showSuccessModal" title="Test Selesai!"
            message="Terima kasih telah menyelesaikan tes jatidiri bri. Anda akan diarahkan ke halaman hasil."
