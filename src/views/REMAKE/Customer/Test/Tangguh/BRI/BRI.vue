    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.length > 0) {
            ...
            testHistory.value.sort((a, b) =>
                new Date(b.assement.created_at).getTime() - new Date(a.assement.created_at).getTime()
            );
...
const navigateToResult = () => {
    if (testHistory.value.length > 0) {
        ...
        localStorage.setItem('bri_id', latestResult.assement.id)
...
const viewTestResult = (result: any) => {
    ...
    localStorage.setItem('bri_id', result.assement.id)
    router.push(`/tangguh/bri/hasil`);
};
        ...
        <div class="w-full min-h-screen p-4 space-y-6 md:max-w-[50%] lg:max-w-[30%] xl:max-w-[25%] mx-auto relative">
            <div v-if="hasTestResult" class="min-h-screen flex flex-col justify-center items-center">
                <div class="text-center mb-8">
                    <h1 class="text-xl font-bold text-[#6464FA] mb-4">Hasil Tes Jatidiri Tangguh - BRI</h1>
                ...
                <div class="w-full md:max-w-lg">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="space-y-6">
                            <div v-for="(result, index) in testHistory" :key="result.bri.id" class="w-full h-20 md:h-24"
                                ...
                                <div class="w-full h-full flex items-center relative">
                                    ...
                                    <div class="w-full bg-white shadow-sm pl-24 md:pl-28 py-4">
                                        ...
                                        <p class="text-sm">Test {{ testHistory.length - index }} <span class="text-[8px] md:text-xs pl-2">{{ formatDate(result.assement.created_at) }}</span></p>
                                    </div>
                                </div>
                            </div>
                        </div>
