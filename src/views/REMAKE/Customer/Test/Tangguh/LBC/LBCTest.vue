...
const STORAGE_KEY = 'lbc_test_progress';
const ANSWERS_KEY = 'lbc_test_answers';
    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.length > 0) {
            ...
            testHistory.value.sort((a, b) =>
                new Date(b.lbc.created_at).getTime() - new Date(a.lbc.created_at).getTime()
            );
...
const fetchQuestions = async () => {
    try {
        ...
        const response = await initAPI('GET', 'lbc/questions', null, token);
...
const submitAnswers = async () => {
    try {
        ...
        const response = await initAPI('POST', 'customers/lbc', payload, token);

        if (response.data && response.data.message === 'Jawaban Berhasil Direkam.') {
            localStorage.removeItem(STORAGE_KEY);
            localStorage.removeItem(ANSWERS_KEY);
            showSuccessModal.value = true;
            localStorage.setItem('lbc_id', response?.data.data.id)
        }
    } catch (err) {
...
<template>
    <div class="w-full bg-[#F8FBFE] font-sora">
        <div class="h-[95vh] p-4 space-y-6 md:max-w-[50%] lg:max-w-[30%] xl:max-w-[25%] mx-auto relative">
            <div v-if="testEligibilityChecked && !canTakeTest"
                ...
                <div class="w-full md:max-w-lg">
                    ...
                    <NewButton @click="navigateToLBC" text="Kembali ke Halaman LBC" class="w-full font-semibold"
                        text-size="text-sm" />
                </div>
            </div>
            ...
            <div v-else-if="testEligibilityChecked && canTakeTest" class="w-full h-full">
                <div class="w-full text-center flex justify-center items-center gap-2 mb-6 md:mb-8">
                    ...
                    <h1 class="text-sm font-bold">Tes Jatidiri Tangguh - LBC</h1>
                </div>
        ...
        <CustomModal :show="showSuccessModal" title="Test Selesai!"
            message="Terima kasih telah menyelesaikan tes jatidiri lbc. Anda akan diarahkan ke halaman hasil."
