...
const STORAGE_KEY = 'mms_test_progress';
const ANSWERS_KEY = 'mms_test_answers';
    ...
    try {
        ...
        if (response.data && response.data.data && response.data.data.length > 0) {
            ...
            testHistory.value.sort((a, b) =>
                new Date(b.mms.created_at).getTime() - new Date(a.mms.created_at).getTime()
            );
...
const fetchQuestions = async () => {
    try {
        ...
        const response = await initAPI('GET', 'mms/questions', null, token);
    ...
    questions.value.forEach(question => {
        ...
        switch (question.type) {
            case 'Kesadaran Diri':
                diri.push(value);
                break;
            case 'Strategi Coping Positif':
                positif.push(value);
                break;
            case 'Keseimbangan Hidup':
                hidup.push(value);
                break;
        }
    });
...
const submitAnswers = async () => {
    try {
        ...
        const response = await initAPI('POST', 'customers/mms', payload, token);

        if (response.data && response.data.message === 'Jawaban Berhasil Direkam.') {
            localStorage.removeItem(STORAGE_KEY);
            localStorage.removeItem(ANSWERS_KEY);
            showSuccessModal.value = true;
            localStorage.setItem('mms_id', response?.data.data.id)
        }
    } catch (err) {
...
<template>
    <div class="w-full bg-[#F8FBFE] font-sora">
        <div class="h-screen p-4 space-y-6 md:max-w-[50%] lg:max-w-[30%] xl:max-w-[25%] mx-auto relative">
            <div v-if="testEligibilityChecked && !canTakeTest"
                ...
                <div class="w-full md:max-w-lg">
                    ...
                    <NewButton @click="navigateToMMS" text="Kembali ke Halaman MMS" class="w-full font-semibold"
                        text-size="text-sm" />
                </div>
            </div>
            ...
            <div v-else-if="testEligibilityChecked && canTakeTest" class="w-full h-full">
                <div class="w-full text-center flex justify-center items-center gap-2 mb-6 md:mb-8">
                    ...
                    <h1 class="text-sm font-bold">Tes Jatidiri Tangguh - MMS</h1>
                </div>
        ...
        <CustomModal :show="showSuccessModal" title="Test Selesai!"
            message="Terima kasih telah menyelesaikan tes jatidiri mms. Anda akan diarahkan ke halaman hasil."
