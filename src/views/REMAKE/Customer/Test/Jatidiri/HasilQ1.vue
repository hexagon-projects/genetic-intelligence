<template>
    ...
    <Layout v-if="!loading">
        <div class="mx-0 lg:mx-[40px] mb-3 h-5 p-7 justify-center items-center gap-2 inline-flex">
            ...
            <div class="opacity-75 text-black text-sm font-normal font-roboto leading-tight">Q1</div>
        </div>
  
        <BelumTest v-if="!isTested" routeUrl="user.views.test_kebahagiaan" message="Test Jatidiri Q1" 
...
import 'sweetalert2/dist/sweetalert2.css';
  ...
  try {
      ...
      const response = await initAPI('get', `customers/q1?customer_id=${userId}`, null, token)
      // console.log(`data iaa`, response.data)

      console.log(response)
      isTested.value = response.data.data ? true : false
      hasilQ1.value = response.data.data ? response.data.data[0].name : null
      descQ1.value = response.data.data ? response.data.data[0].category : null
