<template>
  ...
  <Layout v-if="!loading">
    <div class="mx-0 lg:mx-[40px] mb-3 h-5 p-7 justify-center items-center gap-2 inline-flex">
      ...
      <div class="opacity-75 text-black text-sm font-normal font-roboto leading-tight">
        Q2
      </div>
    </div>

    <SelesaiTest v-if="isTested" routeUrl="user.views.hasil_jatidiri_q2" message="Test Jatidiri Q2 Selesai!"
      ...
      <div class="flex flex-col justify-center items-center gap-[24px] mb-10">
        <div class="flex flex-col items-center gap-[56px]">
          <div class="mx-[30px] md:mx-[60px] h-auto flex-col justify-start items-start gap-4 inline-flex">
            <span
              class="self-stretch text-center text-black text-xl md:text-3xl font-semibold font-['Sora'] leading-9">Tes
              Jatidiri Q2</span>
            <span
              class="self-stretch text-center text-black text-sm md:text-base font-normal font-['Roboto'] leading-normal">Pilihlah
          ...
          <!-- pertanyaan -->
          <div id="soal" v-if="dataPertanyaan"
            class="mx-[30px] md:mx-[60px] w-full h-auto p-9 bg-white rounded-3xl shadow flex flex-col gap-9">
            <span
              class="self-stretch text-center text-black text-xl md:text-3xl font-semibold font-['Sora'] leading-9">Pertanyaan</span>
            <div v-if="!loadingSubmit">
              <div v-for="(q, qIndex) in dataPertanyaan.data" class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="flex flex-col mx-2 mb-5">
                  ...
                  <div class="mt-2 flex items-center justify-center gap-4">
                    <button @click="pilihJawaban(answer.value, qIndex)" v-for="(answer, aIndex) in q.answers"
                      :key="aIndex" :class="{
              ...
              <div class="mt-10 gap-2 flex flex-col md:flex-row justify-center items-center">
                <button @click="handleNextQuestion"
                  ...
                  <div class="text-white text-base font-normal font-['Roboto'] leading-normal justify-center">
                    Kirim Jawaban
                  </div>
...
const subMessage = `Kerja yang bagus! Kamu telah menyelesaikan Tes <span class="font-bold">Jatidiri Q2</span>. Mari lihat hasilnya dan temukan lebih banyak tentang potensi diri Kamu!`;
...
const getUserData = async () => {
  try {
    ...
    isTested.value =
      userData.data.customer.customers_q2 == null ? false : true;
...
const getDataPertanyaan = async (page = 1) => {
  ...
  if (token) {
    try {
      const response = await initAPI("get", `q2/questions`, null, token);
