...
const packageSectionVisible = ref(false);
...
const fetchPackages = async () => {
  try {
    ...
    const response = await initAPI('get', 'user/packages', null, token);
...
const storedBookingData = loadFromStorage(STORAGE_KEY);
if (storedBookingData) {
  selectedDate.value = storedBookingData.selectedDate ? new Date(storedBookingData.selectedDate) : null;
  selectedSlot.value = storedBookingData.selectedSlot || null;
  selectedPackage.value = storedBookingData.selectedPackage || null;
}

watch([selectedDate, selectedSlot, selectedPackage], () => {
...
const navigateToMedical = () => {
  if (!selectedDate.value || !selectedSlot.value || !selectedPackage.value) {
    modalMessage.value = 'Please select date, time and package';
    showErrorModal.value = true;
    return;
  }
...
const triggerAnimations = () => {
  ...
  setTimeout(() => {
    packageSectionVisible.value = true;
  }, 700);
  ...
  <div class="bg-[#F0F7FD] w-full min-h-screen h-full">
    <div
      ...
      <div class="space-y-6 transition-all duration-1000 ease-out"
        :class="packageSectionVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
        <h3 class="text-xl font-medium">Select Package</h3>
        <div v-if="packages.length > 0" class="w-full grid grid-cols-2 gap-4">
          <div v-for="(pkg, index) in packages" :key="pkg.id" @click="selectedPackage = pkg"
            class="p-3 flex flex-col justify-between gap-12 rounded-3xl cursor-pointer transition-all duration-700 ease-out"
            :class="{
              'text-primary bg-[#CBCBFD]': !(selectedPackage && selectedPackage.id === pkg.id),
              'text-white bg-primary': selectedPackage && selectedPackage.id === pkg.id,
              'opacity-100 translate-y-0': packageSectionVisible,
              'opacity-0 translate-y-4': !packageSectionVisible
            }" :style="packageSectionVisible ? { transitionDelay: `${index * 200}ms` } : {}">
            <div>
              <h3 class="text-xl font-medium">{{ pkg.name }}</h3>
              <p class="text-sm">{{ pkg.session_count }} Session{{ pkg.session_count > 1 ? 's' : '' }}</p>
            </div>
            <div>
              <h2 class="text-2xl font-bold">Rp. {{ Math.floor(pkg.price).toLocaleString('id-ID') }}</h2>
            </div>
          </div>
        </div>
        <div v-else class="text-sm text-gray-500">
      ...
      <div class="w-full hidden md:flex items-center gap-4 transition-all duration-1000 ease-out"
        ...
        <button @click="navigateToMedical"
