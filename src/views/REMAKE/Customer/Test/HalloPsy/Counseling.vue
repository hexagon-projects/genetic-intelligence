...
const STORAGE_KEY = 'counselingAnswers';
...
const fetchQuestions = async () => {
  try {
    ...
    const response = await initAPI('get', 'user/hallopsy-questions/counseling', null, token);

    questions.value = response.data.data.counseling;
    questions.value.forEach(q => {
      answers.value[q.id] = '';
    });
  } catch (error) {
    console.error('Error fetching questions:', error);
    modalMessage.value = 'Failed to load counseling questions. Please try again.';
...
const saveAnswers = () => {
  ...
  try {
    ...
    setTimeout(() => {
      router.push('/hallopsy/reservasi/detail/terms');
    }, 1500);
  } catch (error) {
    console.error('Error saving answers:', error);
    modalMessage.value = 'Failed to save counseling information. Please try again.';
    showErrorModal.value = true;
  }
};

// Add this new method to close modals
const closeModal = () => {
  showSuccessModal.value = false;
  showErrorModal.value = false;

  // Navigate to next page if success modal was closed
  ...
  <div class="w-full min-h-screen h-full bg-[#F0F7FD]">
    <div
      ...
      <div class="flex justify-between items-center">
        ...
        <h1 class="text-xl md:text-2xl font-bold">Counseling Information</h1>
        <div></div>
      </div>

      <!-- Loading State -->
      <div v-if="isLoading && questions.length === 0" class="min-h-screen flex justify-center items-center py-10">
        <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary"></div>
      </div>

      <!-- Questions -->
      <div v-for="question in questions" :key="question.id" class="space-y-3">
        <label :for="`question_${question.id}`" class="block text-base font-medium text-gray-800">
          {{ question.question }}
          <span v-if="question.is_required" class="text-red-500">*</span>
        </label>

        <textarea :id="`question_${question.id}`" :name="`question_${question.id}`"
          :placeholder="question.placeholder || 'Your answer...'" :rows="question.rows || 4"
