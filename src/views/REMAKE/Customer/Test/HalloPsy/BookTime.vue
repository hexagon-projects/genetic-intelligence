...
import { format, addMonths, subMonths, parseISO, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, addYears, subYears } from 'date-fns';
...
const generateAvailableDates = () => {
    ...
    availableDates.value = dates.map(date => format(date, 'yyyy-MM-dd'));
...
const proceedToReservation = async () => {
    if (!selectedSlot.value || !selectedPackage.value) {
        modalMessage.value = 'Please select a time slot & package first';
        showErrorModal.value = true;
        return;
    }

    const bookingData = {
        selectedDate: format(selectedDate.value, 'yyyy-MM-dd'),
        selectedTime: selectedSlot.value.start_time,
