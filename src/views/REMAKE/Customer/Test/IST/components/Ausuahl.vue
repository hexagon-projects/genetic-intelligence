<template>
    <div class="space-y-6 relative">
        <div>
            ...
            <div v-if="!showQuestions" class="w-full grid grid-cols-2 gap-4 transition-all duration-700 ease-in-out">
        ...
        <transition name="slide-fade" mode="out-in">
            <div v-if="currentStep === 1 && !showQuestions" key="step1" class="space-y-6 md:space-y-8">
                <div
                    ...
                    <div class="space-y-2 text-center">
                        <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 7</h6>
                        <p class="text-sm text-[#9A9A9A] font-medium">Pertanyaan {{
                            currentQuestion }} dari {{ totalQuestions }}</p>
                    </div>

                    <div class="space-y-4">
                        <h5 class="text-sm">Pada persoalan berikutnya, setiap soal memperlihatkan suatu bentuk tertentu
                            yang
                            terpotong
                            menjadi beberapa bagian.
                            <br />
                            <br />
                            Carilah di antara bentuk-bentuk yang ditentukan (a, b, c, d, e) bentuk yang dibangun dengan
                            cara
                            menyusun potongan-potongan itu sedemikian rupa, sehingga tidak ada kelebihan sudut atau
                            ruang
                            di antaranya.
                        </h5>
                    </div>
                ...
                <div
                    class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                    <div class="space-y-2">
                        <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 7</h6>
                        <p class="text-sm text-[#9A9A9A] font-medium">Contoh Soal</p>
                    </div>

                    <div class="space-y-4">
                        <div class="w-full">
                            <div class="w-[70%] mx-auto h-32 flex items-center justify-center text-center">
                                <img :src="Ausuahl" class="h-32 object-contain">
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap justify-center items-center gap-4">
                            <div v-for="(option, index) in exampleOptions" :key="option.value"
                                ...
                                <img :src="option.img" :alt="option.label"
                                    class="w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 xl:w-20 xl:h-20 object-contain">
                                <p class="text-sm font-medium transition-all duration-300 text-center">{{ option.label
                                    }}
                                </p>
                            </div>
                        </div>
            ...
            <div v-else-if="currentStep === 2 && !showQuestions" key="step3"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2 text-center">
                    <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 7</h6>
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Jawaban yang benar ialah : <b>A</b></p>
                    <p>Oleh karena itu, pilihan A merupakan jawaban yang paling tepat yaitu Bentuk A</p>
                    <div
                        class="w-fit mx-auto p-3 rounded-xl border cursor-pointer transition-all duration-500 text-[#6464FA] border-[#6464FA] bg-[#E5E5FE] transform hover:scale-105">
                        <img :src="A" alt="A"
                            class="w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 xl:w-20 xl:h-20 object-contain">
                        <p class="text-sm font-medium transition-all duration-300 text-center">A
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="showQuestions" key="questions"
                ...
                <div class="space-y-6 md:space-y-8">
                    <div class="w-full">
                        <div class="w-[70%] mx-auto h-32 flex items-center justify-center text-center">
                            <img :src="getImageUrl(currentQuestionData?.question)" class="h-32 object-contain">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap justify-center items-center gap-4">
                        <div v-for="(option, index) in currentQuestionData?.options" :key="option.value"
                            ...
                            <img :src="getImageUrl(option.choice)" :alt="option.value"
                                class="w-14 h-14 md:w-16 md:h-16 lg:w-20 lg:h-20 xl:w-20 xl:h-20 object-contain">
                            <p class="text-sm font-medium transition-all duration-300 text-center">{{ option.label }}
                            </p>
                        </div>
        ...
        <BaseModal v-model="showInfoPopup" title="Petunjuk Soal" :show-header="true" icon="info">
            <p class="text-justify text-sm text-gray-600 mb-4">
                Pada persoalan berikutnya, setiap soal memperlihatkan suatu bentuk tertentu yang terpotong
                menjadi beberapa bagian. Carilah di antara bentuk-bentuk yang ditentukan (a, b, c, d, e) bentuk yang
                dibangun dengan cara
                menyusun potongan-potongan itu sedemikian rupa, sehingga tidak ada kelebihan sudut atau ruang
                di antaranya.
            </p>
...
import Ausuahl from '../../../../../../assets/icons/ist/ausuahl.png'
import A from '../../../../../../assets/icons/ist/a.png'
import B from '../../../../../../assets/icons/ist/b.png'
import C from '../../../../../../assets/icons/ist/c.png'
import D from '../../../../../../assets/icons/ist/d.png'
import E from '../../../../../../assets/icons/ist/e.png'
...
const savedTestState = JSON.parse(localStorage.getItem('auswahlTestState') || '{}')
...
const timer = ref(savedTestState.remainingTime || 420)
const timerInterval = ref(null)
const showTimer = ref(showQuestions.value)

const showInfoPopup = ref(false)
const currentQuestion = ref(117)
const questionsData = ref([])
const isLoading = ref(true)
const showTimeUpModal = ref(false)
const showCompletionModal = ref(false)

const savedAnswers = ref(JSON.parse(localStorage.getItem('auswahlAnswers')) || {})
const savedCurrentQuestion = ref(parseInt(localStorage.getItem('auswahlCurrentQuestion')) || 117)
...
const saveTestState = () => {
    ...
    localStorage.setItem('auswahlTestState', JSON.stringify(testState))
}

const fetchQuestions = async () => {
    try {
        isLoading.value = true
        const response = await initAPI('GET', 'ist/questions?type=FA', null, token)

        if (response.data && response.data.data) {
            questionsData.value = response.data.data.map(item => ({
                id: item.id,
                question: item.questions.replace(/\.PNG$/, '.png'),
                options: item.choices.map((choice, index) => ({
                    value: String.fromCharCode(65 + index),
                    label: `${String.fromCharCode(65 + index)}`,
                    choice: choice.choice.replace(/\.PNG$/, '.png'),
...
const convertAnswersToCorrectValues = () => {
    localStorage.setItem('auswahlAnswersBefore', JSON.stringify(savedAnswers.value))
    ...
    localStorage.setItem('auswahlAnswers', JSON.stringify(convertedAnswers))
...
const setUnansweredToZero = () => {
    ...
    localStorage.setItem('auswahlCompleted', 'true')
}

const exampleOptions = [
    { value: 'A', label: 'A', img: A },
    { value: 'B', label: 'B', img: B },
    { value: 'C', label: 'C', img: C },
    { value: 'D', label: 'D', img: D },
    { value: 'E', label: 'E', img: E }
]
...
const buttonText = computed(() => {
    ...
    return currentStep.value === 2 ? 'Mulai Tes' : 'Selanjutnya'
})
...
const selectAnswer = (value) => {
    ...
    localStorage.setItem('auswahlAnswers', JSON.stringify(savedAnswers.value))
}
...
const goNext = () => {
    if (!showQuestions.value && currentStep.value === 1 && !exampleAnswer.value) {
        return
    }
    ...
    if (showQuestions.value) {
        ...
        if (currentIndex < questionsData.value.length - 1) {
            ...
            localStorage.setItem('auswahlCurrentQuestion', currentQuestion.value.toString())
            saveTestState()
...
const handleTimeUpConfirm = () => {
    localStorage.removeItem('auswahlCurrentQuestion')
    localStorage.removeItem('auswahlTestState')
...
const cleanupTestState = () => {
    localStorage.removeItem('auswahlTestState')
    localStorage.removeItem('auswahlAnswers')
    localStorage.removeItem('auswahlCurrentQuestion')
}
...
watch(() => localStorage.getItem('auswahlCompleted'), (completed) => {
    if (completed === 'true') {
        cleanupTestState()
    }
})
