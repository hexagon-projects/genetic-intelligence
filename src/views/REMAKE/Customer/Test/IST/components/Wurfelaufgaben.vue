        ...
        <transition name="slide-fade" mode="out-in">
            <div v-if="currentStep === 1 && !showQuestions" key="step1" class="space-y-6 md:space-y-8">
                <div
                    ...
                    <div class="space-y-2 text-center">
                        <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 8</h6>
                        <p class="text-sm text-[#9A9A9A] font-medium">Pertanyaan {{
                            currentQuestion }} dari {{ totalQuestions }}</p>
                    </div>

                    <div class="space-y-4">
                        <h5 class="text-sm">Ditentukan 5 (lima) buah kubus a, b, c, d, e. Pada tiap-tiap kubus terdapat
                            enam
                            tanda yang
                            ...
                            kubus-
                            kubus itu dapat mempunyai tanda-tanda yang sama, akan tetapi susunannya berlainan, setiap
                            soal
                            ...
                            saudara, oleh karena itu mungkin akan terlihat suatu tanda yang baru.
                        </h5>
                    </div>
                </div>
                ...
                <div
                    class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                    <div class="space-y-2">
                        <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 8</h6>
                    ...
                    <div class="space-y-6 md:space-y-8">
                        <div class="w-full">
                            <div class="w-[70%] mx-auto h-32 flex items-center justify-center text-center">
                                <img :src="Wuf" class="h-32 object-contain">
                            </div>
                        </div>
            ...
            <div v-else-if="currentStep === 2 && !showQuestions" key="step3"
                ...
                <div class="space-y-4 text-sm">
                    <p>Contoh ini memperlihatkan kubus a dengan kedudukan yang berbeda.
                        Mendapatkannya adalah dengan cara menggulingkan lebih dulu kubus itu ke kiri satu kali dan
                        kemudian diputar ke kiri satu kali, sehingga sisi kubus yang bertanda dua segi empat hitam
                        terletak
                        di depan, seperti kubus a.</p>
                    <div
...
import Wuf from '../../../../../../assets/icons/ist/wuf.png'
import A from '../../../../../../assets/icons/ist/wufa.png'
import B from '../../../../../../assets/icons/ist/wufb.png'
import C from '../../../../../../assets/icons/ist/wufc.png'
import D from '../../../../../../assets/icons/ist/wufd.png'
import E from '../../../../../../assets/icons/ist/wufe.png'
...
const savedTestState = JSON.parse(localStorage.getItem('wurfelaufgabenTestState') || '{}')
...
const timer = ref(savedTestState.remainingTime || 540)
const timerInterval = ref(null)
const showTimer = ref(showQuestions.value)

const showInfoPopup = ref(false)
const currentQuestion = ref(137)
const questionsData = ref([])
const isLoading = ref(true)
const showTimeUpModal = ref(false)
const showCompletionModal = ref(false)

const savedAnswers = ref(JSON.parse(localStorage.getItem('wurfelaufgabenAnswers')) || {})
const savedCurrentQuestion = ref(parseInt(localStorage.getItem('wurfelaufgabenCurrentQuestion')) || 137)
...
const saveTestState = () => {
    ...
    localStorage.setItem('wurfelaufgabenTestState', JSON.stringify(testState))
}

const fetchQuestions = async () => {
    try {
        isLoading.value = true
        const response = await initAPI('GET', 'ist/questions?type=WU', null, token)
...
const convertAnswersToCorrectValues = () => {
    localStorage.setItem('wurfelaufgabenAnswersBefore', JSON.stringify(savedAnswers.value))
    ...
    localStorage.setItem('wurfelaufgabenAnswers', JSON.stringify(convertedAnswers))
...
const setUnansweredToZero = () => {
    ...
    localStorage.setItem('wurfelaufgabenCompleted', 'true')
}
...
const selectAnswer = (value) => {
    ...
    localStorage.setItem('wurfelaufgabenAnswers', JSON.stringify(savedAnswers.value))
}
...
const goNext = () => {
    ...
    if (showQuestions.value) {
        ...
        if (currentIndex < questionsData.value.length - 1) {
            ...
            localStorage.setItem('wurfelaufgabenCurrentQuestion', currentQuestion.value.toString())
            saveTestState()
...
const handleTimeUpConfirm = () => {
    localStorage.removeItem('wurfelaufgabenCurrentQuestion')
    localStorage.removeItem('wurfelaufgabenTestState')
...
const cleanupTestState = () => {
    localStorage.removeItem('wurfelaufgabenTestState')
    localStorage.removeItem('wurfelaufgabenAnswers')
    localStorage.removeItem('wurfelaufgabenCurrentQuestion')
}
...
watch(() => localStorage.getItem('wurfelaufgabenCompleted'), (completed) => {
    if (completed === 'true') {
        cleanupTestState()
    }
})
