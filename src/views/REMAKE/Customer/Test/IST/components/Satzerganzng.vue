    ...
    <div class="space-y-6 relative">
        ...
        <transition name="slide-fade" mode="out-in">
            <div v-if="currentStep === 1 && !showQuestions" key="step1"
                ...
                <div class="space-y-2 text-center">
                    <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 1</h6>
                    <p class="text-sm text-[#9A9A9A] font-medium">Soal 1 - 20</p>
                </div>

                <div class="space-y-4">
                    <h5 class="text-sm">Pada setiap kalimat satu kata hilang dan disediakan 5 (lima) kata
                        pilihan sebagai penggantinya. Pilihan kata yang tepat dapat menyempurnakan kalimat itu !
                        <br />
                        <br />
            ...
            <div v-else-if="currentStep === 2 && !showQuestions" key="step2"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2">
                    <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 1</h6>
                    <p class="text-sm text-[#9A9A9A] font-medium">Contoh Soal</p>
                </div>

                <div class="space-y-4">
                    <h5 class="text-base font-semibold">Seekor kuda mempunyai kesamaan terbanyak dengan seekor .....
                    </h5>
            ...
            <div v-else-if="currentStep === 3 && !showQuestions" key="step3"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Jawaban yang benar ialah : <b>C. Keledai</b></p>
                    <p>Oleh karena itu, pilihan c merupkan jawaban yang paling tepat yaitu Keledai</p>
                    <div
                        ...
                        <p class="text-sm font-medium">C. Kedelai</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="currentStep === 4 && !showQuestions" key="step4"
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold">Lawannya "Harapan" Ialah ....
                    </h5>
            ...
            <div v-else-if="currentStep === 5 && !showQuestions" key="step5"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Jawaban yang benar ialah : <b>B. Putus Asa</b></p>
                    <p>Oleh karena itu, pilihan B merupakan jawaban yang paling tepat yaitu Putus Asa</p>
                    <div
                        class="p-3 rounded-xl border cursor-pointer transition-all duration-500 text-[#6464FA] border-[#6464FA] bg-[#E5E5FE] transform hover:scale-105 animate-bounce-gentle">
                        <p class="text-sm font-medium">B. Putus Asa</p>
                    </div>
                </div>
            </div>
        ...
        <BaseModal v-model="showInfoPopup" title="Petunjuk Soal" icon="info" :show-close-button="true"
            ...
            <p class="text-justify text-sm text-gray-600 mb-4">
                ...
                penggantinya.
                Pilihan kata yang tepat dapat menyempurnakan kalimat itu!
            </p>
...
// Perubahan 1: Load state dari localStorage untuk menentukan apakah harus langsung ke soal
const savedTestState = JSON.parse(localStorage.getItem('satzerganzungTestState') || '{}')
...
const timer = ref(savedTestState.remainingTime || 360) // 6 menit dalam detik
const timerInterval = ref(null)
const showTimer = ref(showQuestions.value) // Langsung show timer jika langsung ke soal
...
const savedAnswers = ref(JSON.parse(localStorage.getItem('satzerganzungAnswers')) || {})
const savedCurrentQuestion = ref(parseInt(localStorage.getItem('satzerganzungCurrentQuestion')) || 1)
...
const saveTestState = () => {
    ...
    localStorage.setItem('satzerganzungTestState', JSON.stringify(testState))
}
...
const convertAnswersToCorrectValues = () => {
    localStorage.setItem('satzerganzungAnswersBefore', JSON.stringify(savedAnswers.value))
    const convertedAnswers = {}

    // Loop melalui semua jawaban yang tersimpan
    Object.keys(savedAnswers.value).forEach(questionNumber => {
        const userAnswer = savedAnswers.value[questionNumber]
        const questionId = parseInt(questionNumber)

        // Cari data soal yang sesuai
        const questionData = questionsData.value.find(q => q.id === questionId)

        if (questionData && userAnswer) {
            // Cari opsi jawaban yang dipilih user
            const selectedOption = questionData.options.find(opt => opt.value === userAnswer)

            // Jika ditemukan, gunakan nilai correct (0/1)
            if (selectedOption) {
                convertedAnswers[questionNumber] = selectedOption.correct
            } else {
                // Jika tidak ditemukan, set sebagai salah (0)
                convertedAnswers[questionNumber] = "0"
            }
        } else {
            // Jika tidak ada jawaban, set sebagai salah (0)
            convertedAnswers[questionNumber] = "0"
        }
    })

    // Simpan hasil konversi ke localStorage
    localStorage.setItem('satzerganzungAnswers', JSON.stringify(convertedAnswers))
    savedAnswers.value = convertedAnswers
}

const handleTimeUpConfirm = () => {
    localStorage.removeItem('satzerganzungCurrentQuestion')
    localStorage.removeItem('satzerganzungTestState')
...
const handleCompletionConfirm = () => {
    ...
    localStorage.setItem('satzerganzungCompleted', 'true')
    emit('test-complete')
}
...
const fetchQuestions = async () => {
    try {
        ...
        const response = await initAPI('GET', 'ist/questions?type=SE', null, token)
        ...
        if (response.data && response.data.data) {
            ...
            currentQuestion.value = savedCurrentQuestion.value
...
const setUnansweredToF = () => {
    for (let i = 1; i <= totalQuestions.value; i++) {
        if (!savedAnswers.value[i]) {
            savedAnswers.value[i] = '0'
        }
    }
...
const exampleOptions = [
    { value: 'A', label: 'A. Sapi' },
    { value: 'B', label: 'B. Kambing' },
    { value: 'C', label: 'C. Keledai' },
    { value: 'D', label: 'D. Rusa' },
    { value: 'E', label: 'E. Kerbau' }
]

const exampleOptions2 = [
    { value: 'A', label: 'A. Duka' },
    { value: 'B', label: 'B. Putus Asa' },
    { value: 'C', label: 'C. Sengsara' },
    { value: 'D', label: 'D. Cinta' },
    { value: 'E', label: 'E. Benci' }
]

const isPrevDisabled = computed(() => {
    if (showQuestions.value && currentQuestion.value === 1) {
        return true
    }
    return currentStep.value === 1 && !showQuestions.value
})
...
const currentQuestionData = computed(() => {
    return questionsData.value.find(q => q.id === currentQuestion.value) || questionsData.value[0]
})
...
const selectAnswer = (value) => {
    ...
    localStorage.setItem('satzerganzungAnswers', JSON.stringify(savedAnswers.value))
}
...
const goNext = () => {
    ...
    if (showQuestions.value) {
        if (currentQuestion.value < totalQuestions.value) {
            ...
            localStorage.setItem('satzerganzungCurrentQuestion', currentQuestion.value.toString())
            saveTestState()
...
const startTimer = () => {
    // Perubahan 6: Timer melanjutkan dari waktu yang tersimpan
    ...
    timerInterval.value = setInterval(() => {
        if (timer.value > 0) {
            timer.value--
            saveTestState() // Simpan waktu yang tersisa
        } else {
            stopTimer()
        }
    }, 1000)
...
watch(() => localStorage.getItem('satzerganzungCompleted'), (completed) => {
    if (completed === 'true') {
        localStorage.removeItem('satzerganzungTestState')
    }
})
