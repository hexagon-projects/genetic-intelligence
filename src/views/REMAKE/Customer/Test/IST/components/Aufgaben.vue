<template>
    <div class="space-y-6 relative">
        ...
        <transition name="slide-fade" mode="out-in">
            <div v-if="currentStep === 1 && !showQuestions" key="step1"
                ...
                <div class="space-y-2 text-center">
                    ...
                    <p class="text-sm text-[#9A9A9A] font-medium">Soal 77 - 96</p>
                </div>
            ...
            <div v-else-if="currentStep === 2 && !showQuestions" key="step2"
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Sebatang pensil harganya 25 rupiah. Berapakah harga
                        3 batang?</h5>
            ...
            <div v-else-if="currentStep === 3 && !showQuestions" key="step3"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Pada contoh soal jawaban ialah 75.
                        Oleh karena itu, kamu harus memilih angka 7 dan 5.
                    </p>
                    ...
                    <div class="text-center">
                        <p class="text-sm text-[#6464FA]">Jawaban yang benar: 75</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="currentStep === 4 && !showQuestions" key="step4"
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Dengan sepeda Husin dapat mencapai 15 km dalam waktu
                        1 jam. Berapa km-kah yang dapat ia capai dalam waktu 4 jam</h5>
            ...
            <div v-else-if="currentStep === 5 && !showQuestions" key="step5"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Pada contoh soal jawaban ialah 60.
                        Oleh karena itu, kamu harus memilih angka 6 dan 0.
                    </p>
                    ...
                    <div class="text-center">
                        <p class="text-sm text-[#6464FA]">Jawaban yang benar: 60</p>
                    </div>
                </div>
            </div>
        ...
        <BaseModal v-model="showInfoPopup" title="Petunjuk Soal" :show-header="true" icon="info">
            <div class="text-justify text-sm text-gray-600 mb-4">
                <p>Kolom ini terdiri atas angka-angka 1 sampai 9 dan 0. Untuk menunjukkan jawaban
                    suatu soal, maka pilihlah angka-angka yang terdapat di dalam jawaban itu.</p>
                <br>
            </div>
...
const savedTestState = JSON.parse(localStorage.getItem('aufgabenTestState') || '{}')
...
const currentQuestionIndex = ref(parseInt(localStorage.getItem('aufgabenCurrentQuestion')) || 0)
const questionsData = ref([])
const savedAnswers = ref(JSON.parse(localStorage.getItem('aufgabenAnswers')) || {})
...
const saveTestState = () => {
    ...
    localStorage.setItem('aufgabenTestState', JSON.stringify(testState))
}

const fetchQuestions = async () => {
    try {
        isLoading.value = true
        const response = await initAPI('GET', 'ist/questions?type=RA', null, token)
        ...
        if (response.data && response.data.data) {
            ...
            if (savedAnswers.value[currentQuestionId]) {
                const savedAnswer = savedAnswers.value[currentQuestionId].userAnswer
...
const setUnansweredToZero = () => {
    ...
    localStorage.setItem('aufgabenAnswers', JSON.stringify(savedAnswers.value))
    localStorage.setItem('aufgabenCompleted', 'true')
}
...
const goNext = () => {
    ...
    if (showQuestions.value) {
        if (currentQuestionIndex.value < questionsData.value.length - 1) {
            ...
            localStorage.setItem('aufgabenCurrentQuestion', currentQuestionIndex.value.toString())
            saveTestState()
...
const handleTimeUpConfirm = () => {
    localStorage.removeItem('aufgabenCurrentQuestion')
    localStorage.removeItem('aufgabenTestState')
...
watch(() => localStorage.getItem('aufgabenCompleted'), (completed) => {
    if (completed === 'true') {
        localStorage.removeItem('aufgabenTestState')
        localStorage.removeItem('aufgabenCurrentQuestion')
    }
