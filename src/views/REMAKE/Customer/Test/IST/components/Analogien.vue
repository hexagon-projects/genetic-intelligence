        ...
        <transition name="slide-fade" mode="out-in">
            <div v-if="currentStep === 1 && !showQuestions" key="step1"
                ...
                <div class="space-y-2 text-center">
                    <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 3</h6>
                    <p class="text-sm text-[#9A9A9A] font-medium">Soal 41 - 60</p>
                </div>

                <div class="space-y-4">
                    <h5 class="text-sm">Antara kata pertama dan kata kedua terdapat suatu hubungan tertentu.
            ...
            <div v-else-if="currentStep === 2 && !showQuestions" key="step2"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2">
                    <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 3</h6>
                    <p class="text-sm text-[#9A9A9A] font-medium">Contoh Soal</p>
                </div>

                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Hutan : pohon = tembok : ?</h5>
            ...
            <div v-else-if="currentStep === 3 && !showQuestions" key="step3"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">hubungan antara hutan dan pohon ialah bahwa hutan terdiri atas pohon-pohon, maka
                        hubungan
                        antara tembok dan salah satu kata pilihan bahwa tembok terdiri atas batu-batu bata.</p>
                    <p>Oleh karena itu, pilihan <b>a</b> merupkan jawaban yang paling tepat yaitu <b>Batu Bata</b></p>
                    <div
                        ...
                        <p class="text-sm font-medium">A. Batu Bata</p>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="currentStep === 4 && !showQuestions" key="step4"
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Gelap : terang = Basah : ?</h5>
            ...
            <div v-else-if="currentStep === 5 && !showQuestions" key="step5"
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Gelap ialah lawannya dari terang, maka untuk basah lawannya ialah kering. Maka jawaban
                        yang benar ialah : e) kering</p>
                    <p>Oleh karena itu, pilihan <b>e</b> merupkan jawaban yang paling tepat yaitu <b>Kering</b></p>
                    <div
                        class="p-3 rounded-xl border cursor-pointer transition-all duration-500 text-[#6464FA] border-[#6464FA] bg-[#E5E5FE] transform hover:scale-105 animate-bounce-gentle">
                        <p class="text-sm font-medium">E. Kering</p>
                    </div>
                </div>
            </div>
...
// State Management
const savedTestState = JSON.parse(localStorage.getItem('analogienTestState') || '{}')
...
const currentQuestion = ref(41) // Default dimulai dari 41 sesuai API
const questionsData = ref([])
const isLoading = ref(true)

// Persisted Data
const savedAnswers = ref(JSON.parse(localStorage.getItem('analogienAnswers')) || {})
const savedCurrentQuestion = ref(parseInt(localStorage.getItem('analogienCurrentQuestion')) || 41)
...
const saveTestState = () => {
    ...
    localStorage.setItem('analogienTestState', JSON.stringify(testState))
}
...
const convertAnswersToCorrectValues = () => {
    localStorage.setItem('analogienAnswersBefore', JSON.stringify(savedAnswers.value))
    ...
    localStorage.setItem('analogienAnswers', JSON.stringify(convertedAnswers))
...
const exampleOptions = [
    ...
    { value: 'D', label: 'D. Putih' },
    { value: 'E', label: 'E. Dinding' }
]

const exampleOptions2 = [
    { value: 'A', label: 'A. Hujan' },
    { value: 'B', label: 'B. Hari' },
    { value: 'C', label: 'C. Lembab' },
    { value: 'D', label: 'D. Angin' },
    { value: 'E', label: 'E. Kering' }
]

// Total questions based on last ID from API
const totalQuestions = computed(() => {
    if (questionsData.value.length === 0) return 60
...
const fetchQuestions = async () => {
    try {
        ...
        const response = await initAPI('GET', 'ist/questions?type=AN', null, token)
        ...
        if (response.data && response.data.data) {
            ...
            // Set currentQuestion to first ID or saved value
            const firstQuestionId = questionsData.value[0]?.id || 41
...
const setUnansweredToZero = () => {
    ...
    localStorage.setItem('analogienCompleted', 'true')
}
...
const selectAnswer = (value) => {
    ...
    localStorage.setItem('analogienAnswers', JSON.stringify(savedAnswers.value))
}
...
const goNext = () => {
    ...
    if (showQuestions.value) {
        ...
        if (currentIndex < questionsData.value.length - 1) {
            ...
            localStorage.setItem('analogienCurrentQuestion', currentQuestion.value.toString())
            saveTestState()
            ...
            localStorage.removeItem('analogienCurrentQuestion')
            localStorage.removeItem('analogienTestState')
            convertAnswersToCorrectValues()
        ...
        if (currentStep.value < 5) {
            ...
            currentQuestion.value = questionsData.value[0]?.id || 41
            startTimer()
        }
        saveTestState()
    }
...
onUnmounted(() => {
    ...
    // Cleanup jika test sudah selesai
    if (localStorage.getItem('analogienCompleted') === 'true') {
        localStorage.removeItem('analogienTestState')
        localStorage.removeItem('analogienCurrentQuestion')
    }
})
...
watch(() => localStorage.getItem('analogienCompleted'), (completed) => {
    if (completed === 'true') {
        localStorage.removeItem('analogienTestState')
    }
})
