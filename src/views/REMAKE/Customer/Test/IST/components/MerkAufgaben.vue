<template>
    <div class="space-y-6 relative">
        <div>
            <div class="w-full flex justify-between items-center">
                <div>
                    ...
                    <div class="text-sm md:text-base font-medium text-primary"
                        v-if="showMemorization && !showQuestions">
                        {{ formattedMemorizationTime }}
                    </div>
                </div>
            ...
            <!-- Progress bars -->
            <div v-if="!showQuestions && !showMemorization"
                class="w-full grid grid-cols-4 gap-4 transition-all duration-700 ease-in-out">
                <div class="relative" v-for="(step, index) in 4" :key="index">
            ...
            <div v-if="showQuestions || showMemorization"
                class="relative w-full transition-all duration-700 ease-in-out">
        ...
        <transition name="slide-fade" mode="out-in">
            <!-- Step 1: Petunjuk -->
            <div v-if="currentStep === 1 && !showMemorization && !showQuestions" key="step1"
                ...
                <div class="space-y-2 text-center">
                    <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 9</h6>
                    <p class="text-sm text-[#9A9A9A] font-medium">Soal {{ currentQuestion }} - {{ totalQuestions }}</p>
                </div>

                <div class="space-y-4">
                    <h5 class="text-sm">Pada persoalan berikutnya, terdapat contoh hafalan dan sejumlah pertanyaan mengenai kata-kata yang
                        harus dihafalkan saudara dalam 3 menit.
                        <br /><br />
                        Lalu Pilihlah jawaban yang terdapat sejumlah pertanyaan mengenai kata-kata yang telah
                        saudara hafalkan tadi.
                    </h5>
                </div>
            </div>

            <div v-else-if="currentStep === 2 && !showMemorization && !showQuestions" key="step2"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2">
                    <div class="space-y-2">
                        <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 9</h6>
                    </div>
                </div>

                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Contoh Hafalan</h5>

                    <div class="space-y-3">
                        <div class="space-y-2" v-for="(category, index) in exampleMemorizationData" :key="index">
                            <div class="py-2 px-5 rounded-full bg-[#E5E5FE] border border-[#CBCBFD] w-fit mx-auto">
                                <h6 class="text-xs font-medium text-center text-primary">{{ category.category }}</h6>
                            </div>
            ...
            <!-- Step 3: Contoh Soal -->
            <div v-else-if="currentStep === 3 && !showMemorization && !showQuestions" key="step3"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2">
                    <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 9</h6>
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold">Kata yang mempunyai huruf permulaan – Q – adalah suatu .......
                    </h5>
            ...
            <!-- Step 4: Jawaban Contoh -->
            <div v-else-if="currentStep === 4 && !showMemorization && !showQuestions" key="step4"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2 text-center">
                    <h6 class="text-base text-black font-semibold">Petunjuk & Contoh Soal Kelompok Sub Tes 9</h6>
                ...
                <div class="space-y-4 text-sm">
                    <p class="">Quintet adalah termasuk dalam jenis kesenian, sehingga jawaban yang benar adalah (d).
                    </p>
                    <div
                        ...
                        <p class="text-sm font-medium">D. Kesenian</p>
                    </div>
                </div>
            </div>

            <!-- Step 5: Hapalan -->
            <div v-else-if="showMemorization && !showQuestions" key="step5"
                class="w-full p-6 bg-white rounded-2xl space-y-4 shadow-xl shadow-black/5 transform transition-all duration-500 hover:shadow-2xl">
                <div class="space-y-2">
                    <div class="w-full flex justify-between items-center">
                        <h6 class="text-sm text-black font-semibold">Kelompok Soal Sub Tes 9</h6>
                ...
                <div class="space-y-4">
                    <h5 class="text-base font-semibold text-center">Hafalkan Kata-Kata di Bawah Ini Selama 3 menit.</h5>

                    <div class="space-y-3">
                        <div class="space-y-2" v-for="(category, index) in memorizationData" :key="index">
                            <div class="py-2 px-5 rounded-full bg-[#E5E5FE] border border-[#CBCBFD] w-fit mx-auto">
                                <h6 class="text-xs font-medium text-center text-primary">{{ category.category }}</h6>
                            </div>
                            <div class="text-sm font-medium text-center">
        ...
        <BaseModal v-model="showInfoPopup" title="Petunjuk Soal" :show-header="true" icon="info">
            <p class="text-justify text-sm text-gray-600 mb-4">
                Pilihlah jawaban yang terdapat sejumlah pertanyaan mengenai kata-kata yang telah
                saudara hafalkan tadi.
            </p>
        ...
        <BaseModal v-model="isLoadingCompleted" :close-on-backdrop="false">
            <div class="flex flex-col items-center">
                <svg class="animate-spin h-8 w-8 text-primary" xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                ...
                <p class="mt-2 text-sm text-gray-600">Mengirim Jawaban...</p>
            </div>
        </BaseModal>
    </div>
</template>
...
const savedTestState = JSON.parse(localStorage.getItem('merkaufgabenTestState') || '{}')
const currentStep = ref(savedTestState.currentStep || 1)
const showMemorization = ref(savedTestState.showMemorization || false)
const showQuestions = ref(savedTestState.showQuestions || false)
const exampleAnswer = ref(null)
const timer = ref(savedTestState.remainingTime || 180)
const memorizationTimer = ref(savedTestState.memorizationTime || 180)
const timerInterval = ref(null)
const memorizationTimerInterval = ref(null)
...
const savedAnswers = ref(JSON.parse(localStorage.getItem('merkaufgabenAnswers')) || {})
const savedCurrentQuestion = ref(parseInt(localStorage.getItem('merkaufgabenCurrentQuestion')))

const token = Cookies.get('token')

const memorizationData = ref([
    ...
    { category: 'Keseniaan', words: ['Quantet', 'Arca', 'Opera', 'Ukiran', 'Gamelan'] },
    { category: 'Binatang', words: ['Rusa', 'Musang', 'Beruang', 'Harimau', 'Zebra'] }
])

const exampleMemorizationData = ref([
    { category: 'Bunga', words: ['Soka', 'Larat', 'Flamboyan', 'Yasmin', 'Dahlia'] },
    { category: 'Perkakas', words: ['Wajan', 'Jarum', 'Kikir', 'Cangkul', 'Palu'] },
    { category: 'Burung', words: ['Itik', 'Elang', 'Walet', 'Tekukur', 'Nuri'] },
    { category: 'Keseniaan', words: ['Quantet', 'Arca', 'Opera', 'Ukiran', 'Gamelan'] },
...
const exampleOptions = [
    { value: 'A', label: 'A. Batu Bata' },
    { value: 'B', label: 'B. Rumah' },
    { value: 'C', label: 'C. Semen' },
    { value: 'D', label: 'D. Kesenian' },
    { value: 'E', label: 'E. Dinding' }
]

const saveTestState = () => {
    const testState = {
        currentStep: currentStep.value,
        showMemorization: showMemorization.value,
        showQuestions: showQuestions.value,
        remainingTime: timer.value,
        memorizationTime: memorizationTimer.value
    }
    localStorage.setItem('merkaufgabenTestState', JSON.stringify(testState))
}

const fetchQuestions = async () => {
    try {
        isLoading.value = true
        const response = await initAPI('GET', 'ist/questions?type=ME', null, token)
        ...
        if (response.data && response.data.data) {
            ...
            const firstQuestionId = questionsData.value[0]?.id
            const lastQuestionId = questionsData.value[questionsData.value.length - 1]?.id

            currentQuestion.value = savedCurrentQuestion.value || firstQuestionId || null
...
const convertAnswersToCorrectValues = () => {
    localStorage.setItem('merkaufgabenAnswersBefore', JSON.stringify(savedAnswers.value))
    ...
    localStorage.setItem('merkaufgabenAnswers', JSON.stringify(convertedAnswers))
...
const setUnansweredToZero = () => {
    ...
    localStorage.setItem('merkaufgabenCompleted', 'true')
    emit('test-complete');
}
...
watch(memorizationTimer, (newValue) => {
    if (newValue === 0 && showMemorization.value) {
        stopMemorizationTimer()
        showMemorization.value = false
...
const totalQuestions = computed(() => {
    if (questionsData.value.length === 0) return 16
...
const isPrevDisabled = computed(() => {
    ...
    if (showMemorization.value) return true
    return currentStep.value === 1
})

const isNextDisabled = computed(() => {
    if (showMemorization.value) return false
    if (showQuestions.value) return !props.selectedAnswer
    if (currentStep.value === 3) return !exampleAnswer.value
    return false
})

const timerProgress = computed(() => {
    if (showMemorization.value) {
        return ((180 - memorizationTimer.value) / 180) * 100
    } else if (showQuestions.value) {
        return ((180 - timer.value) / 180) * 100
    }
    return 0
})
...
const buttonText = computed(() => {
    ...
    return currentStep.value === 4 ? 'Mulai Tes' : 'Selanjutnya'
})
...
const formattedMemorizationTime = computed(() => {
    const minutes = Math.floor(memorizationTimer.value / 60)
    const seconds = memorizationTimer.value % 60
    return `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} Min`
})
...
const selectAnswer = (value) => {
    ...
    localStorage.setItem('merkaufgabenAnswers', JSON.stringify(savedAnswers.value))
}
...
const goNext = () => {
    if (showMemorization.value) {
        ...
        if (currentIndex < questionsData.value.length - 1) {
            ...
            localStorage.setItem('merkaufgabenCurrentQuestion', currentQuestion.value.toString())
            saveTestState()
...
const goBack = () => {
    ...
    } else if (showMemorization.value) {
        return
...
const startMemorizationTimer = () => {
    if (memorizationTimerInterval.value) {
        clearInterval(memorizationTimerInterval.value)
    }

    memorizationTimerInterval.value = setInterval(() => {
        if (memorizationTimer.value > 0) {
...
const stopMemorizationTimer = () => {
    if (memorizationTimerInterval.value) {
        clearInterval(memorizationTimerInterval.value)
        memorizationTimerInterval.value = null
    }
}

const handleTimeUpConfirm = () => {
    localStorage.removeItem('merkaufgabenCurrentQuestion')
    localStorage.removeItem('merkaufgabenTestState')
...
watch(() => localStorage.getItem('merkaufgabenCompleted'), (completed) => {
    if (completed === 'true') {
        localStorage.removeItem('merkaufgabenTestState')
    }
})
