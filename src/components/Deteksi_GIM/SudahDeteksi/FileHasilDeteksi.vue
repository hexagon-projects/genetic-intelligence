<template>
    <div v-if="userResultDetect" class="lg:w-1/3">
        <div class="bg-white rounded-lg shadow-xl p-4 h-full">
            <div class="flex flex-col justify-center items-center">
                <h2 class="font-myFont text-center text-black font-semibold">
                    {{ userResultDetect.gim_result !== null ? 'File Hasil Deteksi' : 'File Kamu Belum Tersedia' }}
                </h2>
                <p class="font-myFont text-center text-gray-500 text-sm mb-4">File kamu sudah tersedia</p>

                <div v-if="userResultDetect.gim_result" class="self-center mb-4">
                    <img @click="showImage" class="w-1/2 mx-auto h-full rounded-lg shadow-xl hover:border-secondary border-2 border-white mb-2" :src="'http://gim.app.api.hexagon.co.id/api/open/results/'+userResultDetect.gim_result" alt="File Deteksi">
                ...
                <div v-if="userResultDetect.gim_result" class="flex flex-col lg:flex-row justify-center items-center gap-2">
                    <button @click="downloadImage" class="min-w-32 px-2 py-2 bg-secondary hover:opacity-75 hover:shadow-lg font-myFont text-white rounded-lg">Download</button>
