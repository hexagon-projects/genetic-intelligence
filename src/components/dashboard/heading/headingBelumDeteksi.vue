    ...
    <div class="lg:w-3/4">
        <div class="gradasi shadow-sm flex flex-col md:flex-row lg:flex-row lg:justify-between items-center rounded-2xl gap-4 px-12 py-12">
            ...
            <div class="w-full flex flex-col gap-4 lg:border-l lg:pl-6 ml-4">
                <div class="w-full">
                    <h1 v-if="userData.is_detected == 'Belum'" class="mb-2 font-myFont font-semibold text-xl lg:text-2xl text-white">
                    ...
                    <RouterLink :to="{name: 'user.views.deteksi'}" v-if="userData.is_detected == 'Belum'" class="w-3/4 lg:w-1/2 flex justiy-center items-center gap-2 px-4 py-2 bg-white bg-opacity-50 backdrop-blur-3xl rounded-lg font-myFont text-dark hover:text-light hover:bg-[#1fabee]">
                        Deteksi Sekarang
                        <PhArrowRight/>
                    </RouterLink>
                    <RouterLink :to="{name: 'user.views.hasil_deteksi'}" v-else-if="userData.is_detected == 'Sudah Disubmit' || userData.is_detected == 'Dalam Review'" class="w-1/2 lg:w-1/3 flex justiy-center items-center gap-2 px-4 py-2 bg-white bg-opacity-50 backdrop-blur-3xl rounded-lg font-myFont text-dark hover:text-light hover:bg-[#1fabee]">
                        Cek Hasil
                        <PhArrowRight/>
                    </RouterLink>
                </div>
