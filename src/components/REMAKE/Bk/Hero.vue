<template>
  <div class="w-full flex flex-col items-center justify-center">
    ...
    <div class="relative w-full">
      <img v-if="!heroImage" src="@/assets/img/login/jatidiri.png" class="object-cover w-full max-h-[400px] rounded-3xl"
        alt="hero" />
      <img v-else :src="heroImage" class="object-cover w-full max-h-[400px] rounded-3xl" alt="hero" />
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4"
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto" @click.stop>
        ...
        <div class="p-6">
          <!-- Upload Area -->
          <div class="border-2 border-dashed rounded-lg p-8 text-center transition-colors" :class="isDragOver ? 'border-blue-500 bg-blue-50' : 'border-gray-300'
        ...
        <div class="flex items-center justify-end gap-3 p-6 border-t bg-gray-50">
          ...
          <button @click="uploadImage" :disabled="!selectedImage || isUploading"
    ...
    <div
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
        <div class="flex gap-3 items-center md:col-span-2">
          <img v-if="(userData.bk || userData.customer)?.profile == null" src="@/assets/img/profile-mock.png"
            class="w-20 h-20 rounded-full outline outline-1 outline-white hidden md:block" alt="profile" />
          ...
          <div class="flex flex-col items-stretch">
            ...
            <h1 class="font-sora text-2xl font-semibold" v-if="userData?.bk || userData?.customer">
...
const uploadImage = async () => {
  ...
  try {
    ...
    // Upload ke server
    const res = await initAPI("post", "bk/upload-hero", formData, token);
