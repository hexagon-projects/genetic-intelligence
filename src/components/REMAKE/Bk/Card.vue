    ...
    <div class="flex flex-col gap-2 mt-5">
      ...
      <div class="flex w-full bg-opacity-10 justify-between items-center">
        ...
        <div class="flex flex-col">
          ...
          <template v-if="userData?.customer">
...
const getLicense = async () => {
  try {
    ...
    if (userData.user.role === 'bk') {
      licenseResponse = await initAPI("get", `bk/license/${userId}`, null, token);
    } else {
      licenseResponse = await initAPI("get", `bk/license-admin/${userId}`, null, token);
    }
