<template>
      ...
      <ul class="flex space-x-8">
            ...
            <img
              :class="{
                'grayscale group-hover:grayscale-0':
                  $route.name !== 'user.views.guru_tk',
                'grayscale-0': $route.name == 'user.views.guru_tk',
              }"
        ...
        >
          <div class="flex items-center gap-[6px]">
            ...
            />
            <span class="text-black font-roboto">Pengguna</span>
          </div>
        </li>

        <li
          @click="goTo('user.views.guru_kuisioner')"
          class="relative group cursor-pointer"
        >
          <div class="flex items-center gap-[6px]">
            <img
              :class="{
                'grayscale group-hover:grayscale-0':
                  $route.name !== 'user.views.guru_kuisioner' ||
                  $route.name !== 'user.views.guru_kuisioner',
                'grayscale-0':
                  $route.name == 'user.views.guru_kuisioner' ||
                  $route.name == 'user.views.guru_kuisioner',
            ...
            />
            <span class="text-black font-roboto">Kuisioner</span>
          </div>
        </li>
      </ul>
  ...
  >
    <div id="tabs" class="flex justify-between">
      <RouterLink
        :to="{ name: 'user.views.guru_tk' }"
        class="w-full flex flex-col justify-center text-center pt-2 pb-1"
        :class="{ 'font-bold': $route.name === 'user.views.guru_tk' }"
      >
      ...
      <RouterLink
        :to="{ name: 'user.views.anak' }"
        class="w-full flex flex-col justify-center text-center pt-2 pb-1"
        :class="{ 'font-bold': $route.name === 'user.views.anak' }"
      >
        <div class="self-center">
          <PhChartBar :size="25" />
        </div>
      ...
      <RouterLink
        :to="{ name: 'user.views.guru_kuisioner' }"
        class="w-full flex flex-col justify-center text-center pt-2 pb-1"
        :class="{ 'font-bold': $route.name === 'user.views.guru_kuisioner' }"
      >
        <div class="self-center">
          <PhUsers :size="25" />
        </div>
        <span class="tab tab-home block text-xs">Kuisioner</span>
      </RouterLink>
    </div>
  </section>
</template>
