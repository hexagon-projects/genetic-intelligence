<template>
    <transition name="fade" mode="out-in">
        <div v-if="showSoalInstruksi" class="fixed z-[999] inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full px-4 modal"
        >
            <SoalInstruksi/>
        </div>
    </transition>
    ...
    <div class="max-w-[700px] flex flex-col items-center justify-center mx-auto shadow rounded-tl-3xl rounded-tr-3xl rounded-b-3xl bg-white">
        <div class="self-stretch h-[100px] py-4 bg-[#3030f8] rounded-tl-3xl rounded-tr-3xl flex-col justify-start items-start gap-2 flex">
            <div class="self-stretch text-center text-white text-3xl font-semibold font-['Sora'] leading-9">Intruksi Pengerjaan Tes IQ</div>
            <div class="self-stretch text-center text-white text-base font-normal font-['Roboto'] leading-normal">Halo Fauzan Muhammad! Sebelum melakukan test perhatikan petunjuk dibawah ini.</div>
        </div>
