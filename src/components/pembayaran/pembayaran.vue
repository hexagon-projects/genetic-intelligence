      ...
      >
        <div class="lg:hidden flex flex-col items-center gap-2">
          ...
          <div class="flex items-center gap-1">
            <PhInfo :size="28" />
            <p class="font-myFont text-neutral-500 text-xs">
              Jika punya, Gunakan kode voucher saat mendaftar untuk meraih
              keuntungan tambahan.
            </p>
          </div>
        </div>
      ...
      try {
        ...
        if (response.data.data && response.data.data.paymentUrl) {
          ...
          localStorage.setItem(
            "merchantId",
            JSON.stringify(response.data.payment_data.merchant_order_id)
          );
          ...
          const updatedCustomer = await initAPI(
            "post",
            "login",
            formData,
            token
          ...
          localStorage.setItem(
            "userData",
            JSON.stringify(updatedCustomer.data.customer)
          );
          ...
          Swal.fire({
            ...
            customClass: {
              confirmButton: "text-white bg-biru font-bold",
            },
            willClose: () => {
              // ini jalan kalau popup ditutup otomatis karena timer
              redirectUser();
            },
          }).then((result) => {
            // ini jalan kalau tombol "Lanjutkan" ditekan
