...
const setActiveTest = (index: number) => {
    ...
    getBRITangguh(props.data[index]?.id);
};
...
const handleDownload = async (id) => {
    ...
    try {
        ...
        const response = await fetch(
            `https://api.jatidiri.app/api/result-bri/${id}`,
            {
                method: 'GET',
                headers: {
...
const getBRITangguh = async (id) => {
    try {
        const token = Cookies.get("token");
        const response = await initAPI(
            "get",
            `customers/bri?customerBriId=${id}`,
            null,
            token
        );
...
onMounted(() => {
    if (props.data && props.data.length > 0) {
        getBRITangguh(props.data[0]?.assement.id);
    }
})
                ...
                <div class="w-full h-full flex items-center relative border border-black/10 rounded-3xl pr-4 cursor-pointer"
                    ...
                    <div class="w-full pl-24 md:pl-28 py-4">
                        ...
                        <p class="text-sm">Test {{ index + 1 }} <span class="text-[8px] md:text-xs pl-2">{{
                            formatDate(item.bri.created_at) }}</span></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="w-full grid grid-cols-3 gap-4">
                <div class="space-y-2 p-3 rounded-2xl bg-[#C2C2FD]">
                    ...
                    <div class="">
                        <h4 class="text-2xl md:text-2xl font-bold">{{ briData?.exhaustion?.percent || '0' }}
                        </h4>
                        <p class="text-[10px] md:text-sm font-normal text-[#606060]">exhaustion</p>
                    </div>
                </div>
                <div class="space-y-2 p-3 rounded-2xl bg-[#C7EFDA]">
                    ...
                    <div class="">
                        <h4 class="text-2xl md:text-2xl font-bold">{{ briData?.depersonalization?.percent ||
                            '0' }}</h4>
                        <p class="text-[10px] md:text-sm font-normal text-[#606060]">depersonalization</p>
                    </div>
                </div>
                <div class="space-y-2 p-3 rounded-2xl bg-[#F7ECBF]">
                    ...
                    <div class="">
                        <h4 class="text-2xl md:text-2xl font-bold">{{ briData?.resilience?.percent || '0' }}
                        </h4>
                        <p class="text-[10px] md:text-sm font-normal text-[#606060]">resilience</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2">
                <h3 class="text-[#111416] text-lg font-bold text-center">
                    Burnout {{ briData?.bri?.name }}
                </h3>
                <p class="" v-html="extractMainDescription(briData?.bri?.description)"></p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <p class="[&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:my-4 [&_li]:my-2"
                    v-html="extractSurvivalSuggestions(briData?.bri?.description)"></p>
            </div>
            ...
            <div class="space-y-2">
                ...
                <p class="" v-html="extractHallopsyInfo(briData?.bri?.description)"></p>
            </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-4 cursor-pointer" @click="toggleDropdown(1)">
                            <p class="text-base text-black font-medium" :class="{ 'text-black': activeDropdown === 1 }">
                                Burnout {{ briData?.burnout?.name || 'Level Burnout' }}
                            </p>
                        ...
                        <div class="border-l border-primary p-2 transition-all duration-300 overflow-hidden"
                            ...
                            <p class="text-base font-medium [&_h3]:font-bold [&_h3]:pt-2 [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:my-4 [&_ol]:list-decimal [&_ol]:pl-5 [&_ol]:my-4 [&_li]:my-2"
                                v-html="briData?.burnout?.description">
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <NewButton :text="downloading ? 'Mengunduh...' : 'Download PDF'" class="font-semibold w-fit px-10"
                ...
                :disabled="downloading" @click="handleDownload(data?.[activeTestIndex]?.assement.id)" />
        </div>
    </section>
</template>